...
function firstMatch(text: string, patterns: RegExp[], unit: string, convert = (n: number, _raw: string) => n): ParsedValue | null {
...
function lengthValue(text: string, labels: string[]): ParsedValue | null {
  ...
  ], 'm', (n, raw) => /cm|厘米/i.test(raw) ? n / 100 : n);
}
...
function velocityValue(text: string, labels: string[]): ParsedValue | null {
  ...
  ], 'm/s', (n, raw) => /km\/h|千米每小时/i.test(raw) ? n / 3.6 : n);
}
...
function chargeValue(text: string): ParsedValue | null {
  ...
  const elementary = firstMatch(text, [
...
function classifyScene(text: string): { sceneId: string; confidence: number; warnings: string[] } {
  ...
  if (ranked.length === 0) {
    return { sceneId: 'projectile', confidence: 0.35, warnings: ['未识别到明确模型关键词，已默认按抛体运动搭建场景。'] };
  }

  const [sceneId, topScore] = ranked[0]!;
  const secondScore = ranked[1]?.[1] ?? 0;
  const confidence = Math.min(0.95, 0.45 + topScore / 20 + Math.max(0, topScore - secondScore) / 30);
  const warnings = secondScore > 0 && topScore - secondScore <= 2
...
function inferParameters(sceneId: string, text: string, assumptions: string[], extracted: ExtractedQuantity[]): Record<string, number> {
