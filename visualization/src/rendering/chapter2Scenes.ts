...
export function drawDoublePendulumSyncScene(opts: Chapter2SceneOptions): void {
  ...
  // 切线方向 (单位向量)
  const ux1 = Math.cos(theta1), uy1 = -Math.sin(theta1);
  const ux2 = Math.cos(theta2), uy2 = -Math.sin(theta2);
...
export function drawForcedVibrationScene(opts: Chapter2SceneOptions): void {
  ...
  // 物块
  draw3DBlock(ctx, blockX, groundY - blockH / 2, blockW, blockH, '#3b82f6', isDark,
    `${m.toFixed(2)}kg`);
  ...
  drawMiniChart({
    ctx,
    x: afChartX, y: afChartY, w: afChartW, h: afChartH,
    xs: afPointsX, ys: afPointsY,
    isDark,
    lineColor: '#3b82f6',
    label: 'A-f 共振曲线',
  ...
  if (xtXs.length > 1) {
    drawMiniChart({
      ctx,
      x: xtChartX, y: xtChartY, w: xtChartW, h: xtChartH,
      xs: xtXs, ys: xtYs,
      isDark,
      lineColor: '#f97316',
      label: '稳态 x-t (末 2 周期)',
...
export function drawResonanceCurveScene(opts: Chapter2SceneOptions): void {
  ...
  if (bottomH > 40) {
    ...
    // 物块
    draw3DBlock(ctx, blockX2, subCY, blockW2, blockH2, '#3b82f6', isDark,
      `${m.toFixed(1)}kg`);
