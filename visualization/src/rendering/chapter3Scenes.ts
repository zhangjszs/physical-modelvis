...
export function drawHookeLawScene(opts: Chapter3SceneOptions): void {
  ...
  draw3DBlock(ctx, springX, blockY + blockH / 2, blockW, blockH, '#3b82f6', isDark,
    `${m.toFixed(2)}kg`);
  ...
  // 重力 (向下，红色)
  drawForceArrow(ctx,
    springX, blockY + blockH,
    springX, blockY + blockH + gravityN * forceScale / 10,
    '#ef4444', 'G=mg', { dx: 8, dy: 0 });
  // 弹簧弹力 (向上，绿色)
  drawForceArrow(ctx,
    springX, blockY,
    springX, blockY - springForceN * forceScale / 10,
    '#22c55e', 'F=kx', { dx: 8, dy: 0 });

  // --- 信息条 ---
  drawInfoBar(ctx, width, height,
...
export function drawSlidingFrictionScene(opts: Chapter3SceneOptions): void {
  ...
  const blockW = 48, blockH = 40;
  const blockCY = groundY - blockH / 2;

  draw3DBlock(ctx, blockScreenX, blockCY, blockW, blockH, '#3b82f6', isDark,
    `${mass}kg`);
  ...
  if (Math.abs(blockV) > 0.01) {
    const vScale = 60;
    drawForceArrow(ctx,
      blockScreenX + blockW / 2, blockCY,
      blockScreenX + blockW / 2 + blockV * vScale, blockCY,
      '#22c55e', 'v', { dx: 4, dy: -16 });
  }

  // --- 受力分析 ---
  const forceScale = 8; // 1N = 8px
  const cx = blockScreenX, cy = blockCY;

  // 重力 G (向下)
  drawForceArrow(ctx,
    cx, cy,
    cx, cy + N * forceScale / 5,
    '#a855f7', 'G=mg', { dx: 8, dy: 0 });
  // 支持力 N (向上)
  drawForceArrow(ctx,
    cx, cy,
    cx, cy - N * forceScale / 5,
  ...
  drawForceArrow(ctx,
    cx + blockW / 2, cy,
    cx + blockW / 2 + F_pull * forceScale, cy,
    '#f59e0b', 'F_pull', { dx: 4, dy: -16 });
  // 摩擦力 f (向左，与运动方向相反)
  drawForceArrow(ctx,
    cx - blockW / 2, cy,
    cx - blockW / 2 - f * forceScale, cy,
    '#ef4444', 'f=μN', { dx: -32, dy: -16 });
...
export function drawForceCompositionScene(opts: Chapter3SceneOptions): void {
  ...
  ctx.fillText(`θ=${angleDeg}°`,
    originX + (arcR + 16) * Math.cos(arcMidAngle),
    originY + (arcR + 16) * Math.sin(arcMidAngle));
  ctx.textBaseline = 'alphabetic';

  // --- 分力 F1 (蓝色, 水平向右) ---
  drawForceArrow(ctx, originX, originY, F1end.x, F1end.y,
    '#3b82f6', `F₁=${F1}N`, { dx: 8, dy: -8 });
  // --- 分力 F2 (红色) ---
  drawForceArrow(ctx, originX, originY, F2end.x, F2end.y,
    '#ef4444', `F₂=${F2}N`, { dx: 8, dy: -8 });
  // --- 合力 F (绿色, 平行四边形对角线) ---
  drawForceArrow(ctx, originX, originY, Fend.x, Fend.y,
    '#22c55e', `F=${Fmag.toFixed(2)}N`, { dx: 8, dy: 8 });
  ...
  if (Math.abs(FangleDeg) > 0.1) {
    ...
    ctx.textAlign = 'left';
    ctx.fillText(`φ=${FangleDeg.toFixed(1)}°`,
      originX + phiArcR + 4, originY - phiArcR);
  }
  ...
  const lineH = 18;
  const boxW = 280, boxH = formulaText.length * lineH + 16;
...
export function drawNewtonThirdLawScene(opts: Chapter3SceneOptions): void {
  ...
  drawForceArrow(ctx,
    bx, blockCY - 20,
    bx + F_AB * forceScale, blockCY - 20,
    '#f59e0b', `F_AB=${F_AB}N`, { dx: 4, dy: -14 });

  // --- 反作用力 F_BA (B 对 A，作用在 A 上，向左) ---
  drawForceArrow(ctx,
    ax, blockCY - 20,
    ax + F_BA * forceScale, blockCY - 20,  // F_BA 是负数，自动向左
    '#a855f7', `F_BA=${F_BA}N`, { dx: -50, dy: -14 });
