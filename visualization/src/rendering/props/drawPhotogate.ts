...
export function drawPhotogate(opts: PhotogateOptions): void {
  const {
    ctx, x, trackTopY, isDark,
    armHeight = 80, armWidth = 6, gapWidth = 36,
    isBlocked = false, ledOn = isBlocked,
    label, labelColor = '#ef4444',
  } = opts;
  ...
  ctx.strokeStyle = ledOn
    ? (isDark ? 'rgba(248,113,113,0.9)' : 'rgba(239,68,68,0.9)')
    : (isDark ? 'rgba(248,113,113,0.3)' : 'rgba(239,68,68,0.3)');
