...
function drawMiniChart(opts: {
  ...
  const { ctx, x, y, w, h, xs, ys, isDark, label, xLabel, yLabel, showPeakX, peakLabel, fillUnder, logX, logY } = opts;
...
function drawArrow(
  ...
  color: string, lineWidth = 2, headLen = 8,
): void {
...
export function drawPhotoresistorScene(o: SensorSceneOptions): void {
  ...
  ctx.font = 'bold 11px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(`R = ${R >= 1e3 ? (R / 1e3).toFixed(1) + ' kΩ' : R.toFixed(1) + ' Ω'}`, symX + symW / 2, symY + symH / 2 + 34);
  ...
  drawMiniChart({
    ctx, x: chartX, y: chartY, w: chartW, h: chartH,
    xs, ys, isDark,
    lineColor: '#fbbf24',
    label: 'R-E 阻值-照度 (双对数)',
    xLabel: '照度 E (lx)',
  ...
  // 阈值线
  const thresholdX = chartX + ((Math.log10(thresholdE) - Math.log10(eMin)) / (Math.log10(eMax) - Math.log10(eMin))) * chartW;
...
export function drawStrainGaugeScene(o: SensorSceneOptions): void {
  ...
  // 右-下 (R₂)
  ctx.beginPath(); ctx.moveTo(corners[1]!.x, corners[1]!.y); ctx.lineTo(corners[2]!.x, corners[2]!.y); ctx.stroke();
  // 下-左 (R₃)
  ctx.beginPath(); ctx.moveTo(corners[2]!.x, corners[2]!.y); ctx.lineTo(corners[3]!.x, corners[3]!.y); ctx.stroke();
  // 左-上 (R₄)
  ctx.beginPath(); ctx.moveTo(corners[3]!.x, corners[3]!.y); ctx.lineTo(corners[0]!.x, corners[0]!.y); ctx.stroke();
  // 输出之间 (从上到下 中间为电压表)
  ctx.beginPath();
  ...
  drawInfoBar(ctx, w, h,
    `惠斯通电桥: ΔR/R=K·ε  ε=${strain}με  K=${K}  ΔU=${(deltaU * 1000).toFixed(3)}mV`,
    isDark);
...
export function drawLightControlSwitchScene(o: SensorSceneOptions): void {
  ...
  for (let ai = 0; ai < 2; ai++) {
    drawArrow(ctx, ldrX2 - 26 + ai * 10, ldrY2 - 24 - ai * 8, ldrX2 - 22 + ai * 10, ldrY2 - 28 - ai * 8, '#fbbf24', 1, 3);
  }
  ctx.fillStyle = isDark ? '#cbd5e1' : '#334155';
  ctx.font = '10px sans-serif';
  ctx.textAlign = 'center';
  ...
  drawMiniChart({
    ctx, x: chartX2, y: chartY2, w: chartW2, h: chartH2,
    xs: xs3, ys: ys3, isDark,
    lineColor: '#fbbf24',
    label: '24h 照度曲线',
    xLabel: '时间 t (h)',
    yLabel: '照度 E (lx)',
  ...
  // 阈值线
  const thrY2 = chartY2 + chartH2 - ((Math.log10(Math.max(0.5, threshold)) - Math.log10(0.5)) / (Math.log10(40000) - Math.log10(0.5))) * chartH2;
  ...
  if (tHours >= 0 && tHours <= 24) {
    ...
    const py3 = chartY2 + chartH2 - ((Math.log10(lvl) - Math.log10(0.5)) / (Math.log10(40000) - Math.log10(0.5))) * chartH2;
    ctx.fillStyle = '#fff';
    ctx.beginPath();
  ...
  // 对数填充
  const slidR2 = Math.max(0, Math.min(1, (Math.log10(Math.max(0.5, L)) - Math.log10(0.5)) / (Math.log10(1e5) - Math.log10(0.5))));
  ctx.fillStyle = '#fbbf24';
