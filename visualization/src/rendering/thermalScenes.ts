...
export function drawDiffusionScene(o: ThermalSceneOptions): void {
  ...
  if (chartH > 60) {
    ...
    if (chart && chart.points.length > 0) {
      ...
      drawMiniChart({
        ctx,
        x: partX0, y: chartY, w: partW, h: chartH,
        xs, ys,
        isDark,
        lineColor: '#3b82f6',
        label: 'C(x) 高斯分布',
...
export function drawBrownianScene(o: ThermalSceneOptions): void {
  ...
  drawInfoBar(ctx, w, h,
    `r=${rUm}μm  T=${T}K  η=${eta}cP  D=${D.toExponential(2)} m²/s  N=${nParts}`,
    isDark);
...
export function drawHeatTransferScene(o: ThermalSceneOptions): void {
  ...
  // ========== 上: 热传导 ==========
  const condY = 55;
  drawHeatTransferPanel(ctx, panelX, condY, panelW, panelH, isDark, '热传导', '#ef4444',
    'Q̇ = k·A·ΔT / L', '分子碰撞传递动能', () => {
      // 沿棒的温度剖面
  ...
  drawHeatTransferPanel(ctx, panelX, convY, panelW, panelH, isDark, '热对流', '#f59e0b',
    'Q̇ = h·A·ΔT', '流体运动传热', () => {
      // 容器
  ...
  drawHeatTransferPanel(ctx, panelX, radY, panelW, panelH, isDark, '热辐射', '#a855f7',
    'Q̇ = ε·σ·A·(T⁴ − T_env⁴)', '电磁波 (无需介质)', () => {
      // 辐射源
  ...
  if (chartH > 60) {
    ...
    if (chartT && chartT.points.length > 0 && chartQ && chartQ.points.length > 0) {
      ...
      if (qMax > 0 && tMax > 0) {
        ...
        if (yMaxT - yMinT < 1e-9) { yMaxT = yMinT + 1; }
  ...
  drawInfoBar(ctx, w, h,
    `T0=${T0}K  Tenv=${Tenv}K  mode=${modeLabel}  傅里叶/牛顿/斯忒藩-玻尔兹曼`,
    isDark);
...
export function drawLiquidCrystalScene(o: ThermalSceneOptions): void {
  ...
  ctx.font = 'bold 11px sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText(`V=${voltage}V ${isOn ? '(开启)' : '(关闭)'}`,
    cellX + cellW / 2, cellY + cellH + 14);
  ctx.textAlign = 'left';
