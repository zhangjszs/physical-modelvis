...
export function drawReactionTimeScene(opts: MechanicsSceneOptions): void {
  ...
  drawInfoBar(ctx, width, height, `夹住距离 h=${targetDistance.toFixed(3)}m  反应时间 t=sqrt(2h/g)=${reaction.toFixed(3)}s`, isDark);
  if (!simulationResult) drawEmptyState(ctx, width, height, isDark);
}
...
export function drawTransmissionBeltScene(opts: MechanicsSceneOptions): void {
  ...
  drawInfoBar(ctx, width, height, `r1=${r1}m  r2=${r2}m  ${mode === 3 ? '同轴: ω1=ω2' : '无打滑: ω1r1=ω2r2'}`, isDark);
}
...
export function drawNewtonSecondLawScene(opts: MechanicsSceneOptions): void {
  ...
  ctx.fillText(`= ${netF.toFixed(2)} / ${mass.toFixed(2)} = ${a.toFixed(2)} m/s²`, width * 0.58 + 118, height * 0.26 + 58);
  drawHud(ctx, isDark, [
