...
function drawArrow(
  ...
  color: string, lineWidth = 2, headLen = 10,
): void {
...
function drawCoilHorizontal(
  ...
  color: string, lineWidth = 2, turns = 6,
): void {
  ctx.strokeStyle = color;
  ctx.lineWidth = lineWidth;
  ctx.lineCap = 'round';
...
export function drawCurrentBalanceScene(opts: EmEquipSceneOptions): void {
  ...
  for (let d = -20; d <= 20; d += 5) {
    ...
    ctx.lineTo(pivotX + (pointerLen + 6 - inward) * Math.sin(-a), dialY + 14 - (pointerLen + 6 - inward) * Math.cos(-a));
    ctx.stroke();
    if (d % 10 === 0) {
      ctx.fillStyle = isDark ? '#94a3b8' : '#334155';
      ctx.font = '9px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(`${d}°`, pivotX + (pointerLen + 14) * Math.sin(-a), dialY + 14 - (pointerLen + 14) * Math.cos(-a));
    }
  }
...
export function drawEmDampingScene(opts: EmEquipSceneOptions): void {
  ...
  if (bottomH > 50) {
    ...
    if (chartY2 + chartH2 < height - 30) {
      ...
      drawMiniChart({
        ctx,
        x: chartX, y: chartY2, w: chartW, h: chartH2,
        xs: ts, ys: thetaFree,
        isDark,
        lineColor: '#a855f7',
        label: '无铝框',
...
export function drawMutualInductanceScene(opts: EmEquipSceneOptions): void {
  ...
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.moveTo(batX, batY); ctx.lineTo(batX, batY + 8);
  ctx.moveTo(batX - 6, batY + 8); ctx.lineTo(batX + 6, batY + 8);
  ctx.moveTo(batX - 3, batY + 14); ctx.lineTo(batX + 3, batY + 14);
  ctx.moveTo(batX, batY + 14); ctx.lineTo(batX, batY + 26);
  ctx.stroke();
...
export function drawLCOscillatorScene(opts: EmEquipSceneOptions): void {
  ...
  ctx.font = 'bold 11px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillText(`电场能 Ee = ${(EeNow * 1e6).toFixed(3)} μJ  (${(eFrac * 100).toFixed(1)}%)`, barX + 8, eBarY + singleBarH / 2 + 4);
  ...
  ctx.font = 'bold 11px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillText(`磁场能 Em = ${(EmNow * 1e6).toFixed(3)} μJ  (${(mFrac * 100).toFixed(1)}%)`, barX + 8, mBarY + singleBarH / 2 + 4);
