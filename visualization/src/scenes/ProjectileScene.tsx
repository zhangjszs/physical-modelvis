...
const PhotogateTimer = lazy(() => import('../components/simulation/PhotogateTimer').then(m => ({ default: m.PhotogateTimer })));
...
export function ProjectileScene() {
  const {
    ...
    simulationResult, setExperimentData,
  } = useSimulationStore();
  ...
  return (
    <div className="scene-container">
      ...
      <div className="main-area">
        <SimulationCanvas />
        <PlaybackControls />
        <Suspense fallback={<div className="panel-section" style={{ padding: 12, color: '#94a3b8', fontSize: 12 }}>加载中…</div>}>
          <GraphPanel />
        </Suspense>
      </div>
      ...
      <div className="sidebar right-sidebar">
        {currentScene === 'air-track' && (
          <Suspense fallback={<div className="panel-section"><div className="panel-title">数字毫秒计</div><div style={{ padding: 12, color: '#94a3b8', fontSize: 12 }}>加载中…</div></div>}>
            <PhotogateTimer />
          </Suspense>
        )}
        <StateInspector />
        <Suspense fallback={<div className="panel-section"><div className="panel-title">诊断报告</div><div style={{ padding: 12, color: '#94a3b8', fontSize: 12 }}>加载中…</div></div>}>
          <DiagnosticsPanel />
        </Suspense>
        <Suspense fallback={<div className="panel-section"><div className="panel-title">公式说明</div><div style={{ padding: 12, color: '#94a3b8', fontSize: 12 }}>加载中…</div></div>}>
          <FormulaPanel />
        </Suspense>
      </div>
    </div>
