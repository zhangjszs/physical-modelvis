...
describe('problemAnalyzer', () => {
  it('builds a projectile scene from a horizontal launch problem', () => {
    const analysis = analyzePhysicsProblem('从距地面20m高处以10m/s的速度水平抛出一小球，取g=9.8m/s²。');
  ...
  it('extracts free-fall height and keeps SI units', () => {
    const analysis = analyzePhysicsProblem('小球从45m高处静止释放，做自由落体运动，求落地时间。');
  ...
  it('converts centimeters and grams for spring oscillator setup', () => {
    const analysis = analyzePhysicsProblem('质量为500g的物体连接劲度系数k=20N/m的弹簧，振幅为10cm，做简谐运动。');
  ...
  it('extracts inclined plane angle and friction coefficient', () => {
    const analysis = analyzePhysicsProblem('物块在倾角30°的粗糙斜面上下滑，摩擦系数μ=0.2，初速度为0m/s。');
  ...
  it('extracts collision parameters and restitution', () => {
    const analysis = analyzePhysicsProblem('甲质量m1=1kg，速度v1=5m/s，乙质量m2=2kg，速度v2=0m/s，发生碰撞，恢复系数e=0.8。');
