...
export class CollisionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (let i = 0; i <= sampleCount; i++) {
      ...
      const pos1 = beforeCollision
        ? x1i + v1i * t
        : x1i + v1i * collisionTime + v1f * (t - collisionTime);
      const pos2 = beforeCollision
        ? x2i + v2i * t
        : x2i + v2i * collisionTime + v2f * (t - collisionTime);
    ...
    const v_t: ChartSeries = {
      ...
      points: [
        ...traj1.map(p => ({ x: p.t, y: p.velocity.x })),
      ],
    };
    const p_t: ChartSeries = {
      xLabel: '时间', yLabel: '动量', xUnit: 's', yUnit: 'kg·m/s',
    ...
    const steps: ExplanationStep[] = [
      ...
      { order: 3, description: '物体1碰后速度', formula: isInelastic ? "v₁' = ((m₁-e·m₂)v₁ + (1+e)m₂v₂) / (m₁+m₂)" : "v₁' = ((m₁-m₂)v₁ + 2m₂v₂) / (m₁+m₂)", calculation: `v₁' = ${v1f.toFixed(4)} m/s` },
      { order: 4, description: '物体2碰后速度', formula: isInelastic ? "v₂' = ((m₂-e·m₁)v₂ + (1+e)m₁v₁) / (m₁+m₂)" : "v₂' = (2m₁v₁ + (m₂-m₁)v₂) / (m₁+m₂)", calculation: `v₂' = ${v2f.toFixed(4)} m/s` },
    ];

    const formulas: FormulaUsage[] = [
      { name: '动量守恒', formula: 'm₁v₁ + m₂v₂ = m₁v₁\' + m₂v₂\'', variables: { m1: { value: m1, unit: 'kg' }, m2: { value: m2, unit: 'kg' }, v1: { value: v1i, unit: 'm/s' }, v2: { value: v2i, unit: 'm/s' } } },
      { name: '碰后速度v₁\'', formula: isInelastic ? "v₁' = ((m₁-e·m₂)v₁ + (1+e)m₂v₂) / (m₁+m₂)" : "v₁' = ((m₁-m₂)v₁ + 2m₂v₂) / (m₁+m₂)", variables: { m1: { value: m1, unit: 'kg' }, m2: { value: m2, unit: 'kg' }, v1: { value: v1i, unit: 'm/s' }, v2: { value: v2i, unit: 'm/s' } } },
    ];
