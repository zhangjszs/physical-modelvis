...
export class ParallelPlateCapacitorModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const C_inv_d: ChartSeries = {
      xLabel: '1/d (m⁻¹)', yLabel: '电容 C (F)', xUnit: 'm⁻¹', yUnit: 'F',
      points: [],
    };
    const dMin = dRange[0]!;
    const dMax = dRange[1]!;
    ...
    const C_S: ChartSeries = {
      xLabel: '面积 S (m²)', yLabel: '电容 C (F)', xUnit: 'm²', yUnit: 'F',
      points: [],
    }
    const sMin = sRange[0]!;
    const sMax = sRange[1]!;
    ...
    const C_epsilonR: ChartSeries = {
      xLabel: '相对介电常数 εr', yLabel: '电容 C (F)', xUnit: '', yUnit: 'F',
      points: [],
    };
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '电容决定式', formula: 'C = εr·S/(4πkd)', variables: { εr: { value: er0, unit: '' }, S: { value: S0, unit: 'm²' }, d: { value: d0, unit: 'm' }, C: { value: C0, unit: 'F' } } },
          { name: '真空介电常数', formula: 'ε₀ = 1/(4πk)', variables: { epsilon0: { value: eps0, unit: 'F/m' } } },
        ],
      },
      errors: [],
      warnings,
