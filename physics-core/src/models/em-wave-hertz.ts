...
export class HertzExperimentModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const currentWave: ChartSeries = {
      xLabel: '时间 t (μs)', yLabel: '振荡电流 i (mA)', xUnit: 'μs', yUnit: 'mA',
    ...
    const emfDistance: ChartSeries = {
      xLabel: '距离 d (m)', yLabel: '接收电动势 ε (mV)', xUnit: 'm', yUnit: 'mV',
    ...
    const standingWave: ChartSeries = {
      xLabel: '位置 x (m)', yLabel: '合成振幅 (归一化)', xUnit: 'm', yUnit: '',
    ...
    const envelope: ChartSeries = {
      xLabel: '位置 x (m)', yLabel: '包络', xUnit: 'm', yUnit: '',
      points: Array.from({ length: 100 }, (_, i) => {
    ...
    return {
      meta: { model: 'em-wave-hertz', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: 'LC 振荡频率', formula: 'f=1/(2π√(LC))', variables: { f: { value: f, unit: 'Hz' }, λ: { value: lambda, unit: 'm' } } },
          { name: '接收电动势', formula: 'ε∝N·f/d²', variables: { ε: { value: currentEmf, unit: 'V' }, d: { value: distance, unit: 'm' } } },
          { name: '驻波', formula: 'y=2A·cos(2πx/λ)·cos(2πft)', variables: { λ: { value: lambda, unit: 'm' } } },
        ],
      },
      errors: [],
      warnings,
