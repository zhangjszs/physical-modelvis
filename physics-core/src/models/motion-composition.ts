...
export class MotionCompositionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (const f of fractions) {
      const tF = duration * f;
      const idx = Math.round(f * sampleCount);
    ...
    const vx_t: ChartSeries = {
      xLabel: '时间', yLabel: '水平分速度 (恒量)', xUnit: 's', yUnit: 'm/s',
      points: trajectory.map(p => ({ x: p.t, y: p.velocity.x })),
    };
    // 轨迹方程图 (y-x 抛物线)
    const y_x: ChartSeries = {
      xLabel: 'x', yLabel: 'y 轨迹', xUnit: 'm', yUnit: 'm',
    ...
    const formulas: FormulaUsage[] = [
      ...
      { name: '竖直分运动', formula: 'y = ½·a_y·t², vy = a_y·t', variables: { 'a_y': { value: ay, unit: 'm/s²' } } },
      { name: '合速度大小', formula: 'v_合 = √(vx² + vy²)', variables: { vx: { value: vxConst, unit: 'm/s' }, vy: { value: finalVy, unit: 'm/s' }, v: { value: finalV, unit: 'm/s' } } },
      { name: '合速度方向', formula: 'tanθ = vy/vx', variables: { θ: { value: Math.atan2(finalVy, vxConst) * 180 / Math.PI, unit: '°' } } },
    ];
