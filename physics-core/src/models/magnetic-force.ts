...
export class MagneticForceModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const F_theta: ChartSeries = {
      xLabel: '夹角 θ (°)', yLabel: '力 F (N)', xUnit: '°', yUnit: 'N',
      points: [],
    };
    ...
    if (radius > 0 && period > 0) {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        circularPath.push({
          ...
          velocity: { x: parseFloat((-radius * Math.sin(omega_t) / period * 2 * Math.PI).toFixed(5)), y: parseFloat((radius * Math.cos(omega_t) / period * 2 * Math.PI).toFixed(5)) },
    ...
    if (hasAmpere) {
      stepsExpl.push({ order: 1, description: '安培力公式', formula: 'F = B·I·L·sinθ', calculation: `F = ${B} × ${mc.current} × ${mc.wireLength} × sin${ampereForceTheta}° = ${ampereForce.toFixed(2)} N` });
      stepsExpl.push({ order: 2, description: '左手定则', formula: '磁穿掌心, 四指电流, 拇指 = F', result: 'F 垂直于 B 与 I 所确定的平面' });
    }
    if (hasLorentz) {
      stepsExpl.push({ order: hasAmpere ? 3 : 1, description: '洛伦兹力公式', formula: 'F = |q|·v·B·sinφ', calculation: `F = ${Math.abs(mc.charge!)} × ${mc.velocity} × ${B} × sin${lorentzAngle}° = ${lorentzForce.toFixed(2)} N` });
      stepsExpl.push({ order: hasAmpere ? 4 : 2, description: '洛伦兹力不做功', formula: 'F⊥v → P = F·v = 0', result: '只改变速度方向, 不改变速率' });
      if (radius > 0) {
        stepsExpl.push({ order: 5, description: '匀速圆周运动', formula: 'qvB = mv²/r → r = mv/(qB), T = 2πm/(qB)', calculation: `r=${radius.toFixed(4)} m, T=${period.toFixed(4)} s` });
      }
    }
    ...
    const trajectories = [circularPath.length > 0 ? circularPath : [
      { t: 0, position: { x: 0, y: 0 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 },
    ]];

    return {
      meta: { model: 'magnetic-force', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories,
      keyframes,
      charts: { x_t: F_theta },
      diagnostics: {
        conservedQuantities: [],
        maxValues: { magneticField: B, ampereForce, lorentzForce, radius, period, hasAmpere: hasAmpere ? 1 : 0, hasLorentz: hasLorentz ? 1 : 0 },
      ...
      explanation: {
        ...
        steps: stepsExpl,
        formulas: [
          { name: '安培力', formula: 'F=BIL·sinθ', variables: { B: { value: B, unit: 'T' }, F: { value: ampereForce, unit: 'N' } } },
          { name: '洛伦兹力', formula: 'F=|q|vB·sinφ', variables: { B: { value: B, unit: 'T' }, F: { value: lorentzForce, unit: 'N' } } },
