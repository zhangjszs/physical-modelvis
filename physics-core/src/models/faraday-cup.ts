...
export class FaradayCupModel extends PhysicsModelBase {
    ...
    readonly errorSources = ['实际圆筒有厚度, 不完全是等势体', '边缘效应导致局部电荷非零', '探针接触时电荷重新分布'];
    readonly requiredParameters: ParameterSpec[] = [
        {
            name: 'innerProbeDepth',
            description: '内壁探针深度 (0=刚好内壁, 1=腔体深处)',
            unit: '',
            required: true,
            min: 0,
            max: 1
        },
        {
            ...
            description: '外壁探针接触深度 (0=刚好表面, 1=外侧)',
            unit: '',
            required: true,
            min: 0,
            max: 1
        ...
        const steps: ExplanationStep[] = [
            {
                order: 1,
                description: '法拉第定律',
                formula: '空腔导体电荷仅分布外表面',
                result: `Q_内表面=0, Q_外表面=${Q} μC`
            },
            {
                order: 2,
                description: '内壁探针测量',
                formula: 'q_内探针 = 0 (内表面无电荷)',
                calculation: `测量值=${innerMeasurement} μC`
            },
            {
                order: 3,
                description: '外壁探针测量',
                formula: 'q_外探针 = Q_总',
                calculation: `测量值=${outerMeasurement} μC`
            },
        ...
        return {
            ...
            explanation: {
                ...
                formulas: [
                    {
                        name: '法拉第定律',
                        formula: 'Q_内表面 = 0, Q_外表面 = Q_总',
                        variables: { Q: { value: Q, unit: 'μC' } }
                    }
                ]
            },
            errors: [],
