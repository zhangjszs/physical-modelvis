...
export class NeutronDiscoveryModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const E_alpha = c.alphaEnergy, M = c.targetMass;
    const m_alpha = 4.0;
    const v_alpha = Math.sqrt(2 * E_alpha / m_alpha);

    const vH = (2 * m_alpha / (m_alpha + 1)) * v_alpha;
    const vN = (2 * m_alpha / (m_alpha + 14)) * v_alpha;
    const v_n_H = (m_alpha - 1) / (m_alpha + 1) * v_alpha;
    const v_n_N = (m_alpha - 14) / (m_alpha + 14) * v_alpha;
    ...
    const m_n_est = m_alpha * (E_alpha / KE_H);

    const x_t: ChartSeries = { xLabel: '靶核质量 (u)', yLabel: '反冲核速度 (a.u.)', xUnit: 'u', yUnit: '', points: [] };
    ...
    for (let M_i = 1; M_i <= 14; M_i++) {
      ...
      if (M_i === 1 || M_i === 14) y_t.points.push({ x: M_i === 1 ? 1 : 2, y: parseFloat((0.5 * M_i * v_rec * v_rec / v_alpha / v_alpha).toFixed(3)) });
      trajectory.push({ t: M_i * 0.1, position: { x: M_i, y: v_rec }, velocity: { x: 0, y: 0 } });
    }

    return {
      meta: { model: 'neutron-discovery', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '氢靶', t: 0, position: { x: 1, y: vH }, velocity: { x: 0, y: 0 }, description: `KE_H=${KE_H.toFixed(2)}MeV` },
        { label: '氮靶', t: 0, position: { x: 14, y: vN }, velocity: { x: 0, y: 0 }, description: `KE_N=${KE_N.toFixed(2)}MeV` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { E_alpha, M, KE_H, KE_N, ratioKE }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `中子发现: E_α=${E_alpha}MeV, M=${M}u, KE_H/KE_N≈(14)^2`, steps: [
        { order: 1, description: '弹性正碰', formula: 'v_recoil = 2m_α/(m_α+M) · v_α', result: '氢靶反冲速度 ≈ 氮靶的 14 倍' },
        { order: 2, description: '中子质量推断', formula: 'm_n ≈ m_H (动能比值)', result: `KE_H/KE_N ≈ 14² → m_n ≈ m_p` },
      ], formulas: [{ name: '反冲速度', formula: 'v_recoil = 2m_αv_α/(m_α+M)', variables: { m_alpha: { value: m_alpha, unit: 'u' } } }] },
      errors: [], warnings: [],
    };
  }
}
