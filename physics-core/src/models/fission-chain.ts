...
export class FissionChainModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const k = c.multiplicationFactor, genMax = c.generations, N0 = c.initialNeutrons ?? 1;
    ...
    return {
      meta: { model: 'fission-chain', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '当前状态', t: 0, position: { x: genMax, y: N0 * Math.pow(k, genMax) }, velocity: { x: 0, y: 0 }, description: `k=${k}, ${kStatus}` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { k, genMax, Nfinal: N0 * Math.pow(k, genMax), totalFissions: total }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `链式反应: k=${k}, N0=${N0}, gen=${genMax}, ${kStatus}, E≈${(E_tot / 1e6).toFixed(1)}MJ`, steps: [
        { order: 1, description: '增殖公式', formula: 'N_g = N0 · k^g', result: `N_${genMax} = ${N0} × ${k}^${genMax} = ${(N0 * Math.pow(k, genMax)).toFixed(2)}` },
        { order: 2, description: '临界条件', formula: 'k=1 临界; k>1 超临界; k<1 次临界', result: kStatus },
      ], formulas: [{ name: '增殖', formula: 'N_g = N0·k^g', variables: { k: { value: k, unit: '' }, genMax: { value: genMax, unit: '' } } }] },
      errors: [], warnings: [],
    };
  }
}
