...
export class EddyCurrentModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    // 等效涡流电阻 (近似)
    const R_equiv = (6 * rho) / (pi2 * B * B * f * f * d * d * (area / (2 * Math.PI * Math.sqrt(area / Math.PI))));
    ...
    const keyframes: Keyframe[] = [
      ...
      },
      {
        label: '高频 (f/δ>>1)',
        t: duration / 2,
        position: { x: Math.min(f * 10, fMax), y: (pi2 * B * B * Math.pow(Math.min(f * 10, fMax), 2) * d * d * V) / (6 * rho) },
        velocity: { x: 0, y: 0 },
    ...
    if (skinDepth < d / 10) {
      warnings.push(`趋肤深度 ${(skinDepth * 1000).toFixed(3)}mm 远小于厚度 ${(d * 1000).toFixed(3)}mm，内部几乎无涡流`);
    }
    if (skinDepth > d * 10) {
      warnings.push(`趋肤深度 ${(skinDepth * 1000).toFixed(3)}mm 远大于厚度 ${(d * 1000).toFixed(3)}mm，平板公式可能偏差`);
    }
    if (tau_thermal < 1) {
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '涡流功率', formula: 'P = π²B²f²d²V/(6ρ)', variables: { B: { value: B, unit: 'T' }, f: { value: f, unit: 'Hz' }, d: { value: d, unit: 'm' }, V: { value: V, unit: 'm³' }, rho: { value: rho, unit: 'Ω·m' }, P: { value: P, unit: 'W' } } },
          { name: '趋肤深度', formula: 'δ = √(ρ/(πfμ))', variables: { rho: { value: rho, unit: 'Ω·m' }, f: { value: f, unit: 'Hz' }, mu: { value: mu, unit: 'H/m' }, delta: { value: skinDepth, unit: 'm' } } },
        ],
      },
      errors: [],
      warnings,
