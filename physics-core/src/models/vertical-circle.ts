...
export class VerticalCircleModel extends PhysicsModelBase {
  ...
  static modelLabel(t: VerticalCircleType): string {
    switch (t) {
      case 'rope': return '绳模型';
      case 'rod': return '杆模型';
      case 'ring': return '圆环模型';
    }
  }
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const isFullCircle = modelType === 'rod'
      ? v0 >= 0 // 杆总可完成圆周
    ...
    // 对于杆: 完整圆周；对于绳/环: 只有 v₀ ≥ √(5gr) 才做完整圆周
    const totalAngle = isFullCircle
      ? omega * duration  // 完整圆周 (可能有 N 反向)
      : Math.PI; // 只模拟到最高点附近
    const phi0 = -Math.PI / 2; // 最低点时物体在 (0, y=0) 相对于圆心

    const angleSpan = isFullCircle ? omega * duration : Math.PI;
    ...
    const effectiveAngle = isFullCircle ? omega * duration : Math.min(omega * duration, thetaMax);
    ...
    for (let i = 0; i <= sampleCount; i++) {
      ...
      let constraintForce: number;
      if (modelType === 'rod') {
        // 杆: N = m·v²/r - mg·cosθ (正值=向外拉, 负值=向上支持)
        constraintForce = m * (vActual * vActual / r - g * cosTheta);
      } else {
        // 绳/环: T = m·v²/r + mg·cosθ (只能 ≥0)
        constraintForce = vSq > 0
          ? m * (vActual * vActual / r + g * cosTheta)
          : 0;
      }
    ...
    const tension_angle: ChartSeries = {
      xLabel: '角度 θ (0=最低点, π=最高点)', xUnit: 'rad', yLabel: modelType === 'rod' ? '杆力 N (N)' : '绳张力 T (N)', yUnit: 'N',
    ...
    const vc_speed_angle: ChartSeries = {
      xLabel: '角度 θ', xUnit: 'rad', yLabel: '速率 v', yUnit: 'm/s',
      points: Array.from({ length: thetaSteps }, (_, i) => {
        const theta = (2 * Math.PI * i) / (thetaSteps - 1);
        const cosTheta = Math.cos(theta);
        const vSq = v0 * v0 - 2 * g * r * (1 - cosTheta);
    ...
    const vc_energy_angle: ChartSeries = {
      xLabel: '角度 θ', xUnit: 'rad', yLabel: '动能/势能', yUnit: 'J',
      points: Array.from({ length: thetaSteps }, (_, i) => {
        const theta = (2 * Math.PI * i) / (thetaSteps - 1);
        const cosTheta = Math.cos(theta);
        const vSq = v0 * v0 - 2 * g * r * (1 - cosTheta);
    ...
    const vmin_markers: ChartSeries = {
      xLabel: '半径 r', xUnit: 'm', yLabel: '临界速度 v_min', yUnit: 'm/s',
    ...
    // 受力分析图: 最高点
    const topForce: number = modelType === 'rod'
      ? m * (Math.max(0, vTopSq) / r - g)
      : (vTopSq > 0 ? m * (vTopSq / r + g) : 0);
    const forceDiagram: ForceDiagram = {
      bodyId: problem.bodies[0]?.id ?? 'ball',
      forces: [
        { name: modelType === 'rod' ? '杆力 N' : '绳/环张力 T', vector: { x: 0, y: topForce }, magnitude: Math.abs(topForce), unit: 'N' },
    ...
    const maxDev = energies.length > 0
      ? Math.max(...energies.map(e => Math.abs(e - E0)))
      : 0;
    ...
    return {
      ...
      explanation: {
        summary,
        steps: modelType === 'rod' ? [
          { order: 1, description: '杆模型: 杆对物体既可提供向下拉力也可提供向上支持力', formula: 'N(θ) = m(v²/r − g·cosθ)' },
          { order: 2, description: '最高点 (θ=π) 临界条件: v=0 时 N = −mg (↑), 刚好支持', formula: 'N_top = m(v_top²/r − g)', calculation: `v_top = ${vTopActual.toFixed(2)} m/s, N_top = ${topForce.toFixed(2)} N` },
          { order: 3, description: '最小速度 (杆模型): v_min = 0', formula: 'v_min = 0', result: '杆总可以让物体完成完整圆周' },
          { order: 4, description: '机械能守恒验证', formula: '½mv₀² = ½mv(θ)² + mgr(1−cosθ)', result: '最高点 v = √(v₀² − 4gr)' },
        ] : [
          { order: 1, description: `${modelLabel}: 约束只能提供指向圆心的拉力/支持力`, formula: 'T(θ) = m(v²/r + g·cosθ) ≥ 0' },
          { order: 2, description: '最高点临界: T=0 → mg = mv²/r → v_min = √(gr)', formula: 'v_min = √(gr)', calculation: `v_min = √(${g}·${r}) = ${vMin.toFixed(2)} m/s` },
          { order: 3, description: '完成完整圆周的最低速度: v₀ ≥ √(5gr)', formula: 'v₀_min = √(5gr)', calculation: `v₀_min = √(5·${g}·${r}) = ${Math.sqrt(5 * g * r).toFixed(2)} m/s, 当前 v₀ = ${v0.toFixed(2)} m/s` },
          { order: 4, description: `当前状态: ${passesTop ? 'v_top ≥ v_min → 能过最高点' : 'v_top < v_min → 松手做斜抛'}`, formula: `v_top = ${vTopActual.toFixed(2)} m/s ${passesTop ? '≥' : '<'} v_min = ${vMin.toFixed(2)} m/s` },
        ],
        formulas: [
          { name: '最高点临界速度', formula: modelType === 'rod' ? 'v_min = 0' : 'v_min = √(gr)', variables: { 'v_min': { value: vMin, unit: 'm/s' }, g: { value: g, unit: 'm/s²' }, r: { value: r, unit: 'm' } } },
          { name: '机械能守恒', formula: '½mv₀² = ½mv(θ)² + mgr(1−cosθ)', variables: { 'v₀': { value: v0, unit: 'm/s' }, 'v_top': { value: vTopActual, unit: 'm/s' } } },
          { name: modelType === 'rod' ? '杆力公式' : '绳张力公式', formula: modelType === 'rod' ? 'N = m(v²/r − g·cosθ)' : 'T = m(v²/r + g·cosθ)', variables: { m: { value: m, unit: 'kg' }, 'v_top': { value: vTopActual, unit: 'm/s' } } },
        ],
      },
      errors: [],
      warnings: [],
