...
export class TransmissionBeltModel extends PhysicsModelBase {
    ...
    readonly assumptions = ['皮带无打滑、无伸长', '齿轮啮合点线速度相等', '两轮匀速转动 (稳态传动)', '轴摩擦可忽略'];
    readonly applicableRange = '主动轮与从动轮之间通过皮带/齿轮/摩擦/同轴连接的系统';
    readonly errorSources = ['实际皮带存在伸缩、打滑', '齿轮啮合间隙 (背隙)', '轴承摩擦影响角速度微小衰减'];
                ...
                steps:
                    mode === 'coax'
                        ? [
                              {
                                  order: 1,
                                  description: `${modeLabel}: 主从轮固定在同一转轴上`,
                                  formula: 'ω₁ = ω₂',
                                  calculation: `ω₁ = ω₂ = ${omega1.toFixed(2)} rad/s`
                              },
                              {
                                  order: 2,
                                  description: '同轴转动 → 角速度相等、转向相同',
                                  formula: 'ω_common = const',
                                  result: `ω_common = ${omega1.toFixed(2)} rad/s`
                              },
                              {
                                  order: 3,
                                  description: '边缘线速度 (半径不同故 v 不等)',
                                  formula: 'v = ω·r',
                                  calculation: `v₁ = ${omega1.toFixed(2)}·${r1} = ${(omega1 * r1).toFixed(2)} m/s, v₂ = ${omega2.toFixed(2)}·${r2} = ${(omega2 * r2).toFixed(2)} m/s`
                              }
                          ]
                        : [
                              {
                                  order: 1,
                                  description: `${modeLabel}: ${modeDesc}`,
                                  formula: mode === 'gear' ? 'ω₁·r₁ = ω₂·r₂, 反向' : 'ω₁·r₁ = ω₂·r₂, 同向'
                              },
                              {
                                  order: 2,
                                  description: '角速度比',
                                  formula: '|ω₂/ω₁| = r₁/r₂',
                                  calculation: `|ω₂/ω₁| = ${gearRatioAbs.toFixed(3)}, r₁/r₂ = ${radiusRatio.toFixed(3)}`
                              },
                              {
                                  order: 3,
                                  description: '边缘线速度等大',
                                  formula: 'v = ω·r',
                                  calculation: `v₁ = ${omega1.toFixed(2)}·${r1} = ${vSurface.toFixed(2)} m/s, v₂ = ${omega2.toFixed(2)}·${r2} = ${vSurface2.toFixed(2)} m/s`
                              },
                              {
                                  order: 4,
                                  description: '周期',
                                  formula: 'T = 2π/ω',
                                  calculation: `T₁ = ${T1.toFixed(3)}s, T₂ = ${T2.toFixed(3)}s`
                              }
                          ],
                formulas: [
                    {
                        ...
                        formula: 'v = ω·r',
                        variables: {
                            v: { value: vSurface, unit: 'm/s' },
                            'ω₁': { value: Math.abs(omega1), unit: 'rad/s' },
                            'r₁': { value: r1, unit: 'm' }
                        }
                    },
                    {
                        ...
                        formula: '|ω₂/ω₁| = r₁/r₂',
                        variables: {
                            'ω₂/ω₁': { value: gearRatioAbs, unit: '' },
                            'r₁/r₂': { value: radiusRatio, unit: '' }
                        }
                    },
                    {
                        name: '周期',
                        formula: 'T = 2π/ω',
                        variables: { 'T₁': { value: T1, unit: 's' }, 'T₂': { value: T2, unit: 's' } }
                    }
                ]
            },
            errors: [],
