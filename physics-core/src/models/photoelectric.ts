...
export class PhotoelectricModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const Uc_nu: ChartSeries = {
      xLabel: '入射光频率 ν (THz)', yLabel: '截止电压 U_c (V)', xUnit: 'THz', yUnit: 'V',
      points: [],
    };
    const N = 200;
    for (let i = 0; i <= N; i++) {
    ...
    const Ek_nu: ChartSeries = {
      xLabel: '入射光频率 ν (THz)', yLabel: '最大初动能 E_k (eV)', xUnit: 'THz', yUnit: 'eV',
      points: [],
    };
    for (let i = 0; i <= N; i++) {
      const nu = freqMin + (freqMax - freqMin) * i / N;
    ...
    const keyframes: Keyframe[] = [
      ...
      {
        label: '典型频率点', t: 0,
        position: { x: parseFloat((nu0_THz * 1.3).toFixed(2)), y: parseFloat(((h * nu0 * 1.3 - W0_J) / e).toFixed(2)) },
        velocity: { x: 0, y: 0 },
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '光电方程', formula: 'hν = W₀ + eU_c', variables: { h: { value: h, unit: 'J·s' }, W0: { value: W0_eV, unit: 'eV' }, e: { value: e, unit: 'C' } } },
