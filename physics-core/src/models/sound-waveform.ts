...
export class SoundWaveformModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'harmonics', description: '谐波数组 (仅 complex 模式), 例 [0.5, 0.3, 0.2] 表示2倍频0.5, 3倍频0.3...', unit: '', required: false },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const steps: ExplanationStep[] = [
      {
        order: 1,
        description: '波形类型',
        formula: waveType === 'pure' ? 'x(t) = A*sin(2*pi*f*t)' : waveType === 'complex' ? 'x(t) = A*sin(2*pi*f*t) + sum(A_n*sin(2*pi*n*f*t))' : 'x(t) = random (伪随机)',
        result: waveLabel,
      },
      {
        order: 2,
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '简谐', formula: 'x(t) = A*sin(2*pi*f*t)', variables: { A: { value: A, unit: '' }, f: { value: f, unit: 'Hz' } } },
