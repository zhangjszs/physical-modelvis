  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const steps: ExplanationStep[] = [
      ...
      },
      {
        ...
        formula: mode === 'turnOn'
          ? 'i(t) = I_ss * (1 - exp(-t/tau))'
          : 'i(t) = I0 * exp(-t/tau)',
        calculation: mode === 'turnOn'
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '时间常数', formula: 'tau = L/R', variables: { L: { value: L, unit: 'H' }, R: { value: R, unit: 'Ohm' }, tau: { value: tau, unit: 's' } } },
          { name: mode === 'turnOn' ? '通电电流' : '断电电流', formula: mode === 'turnOn' ? 'i=E/R*(1-e^{-t/tau})' : 'i=I0*e^{-t/tau}', variables: { E: { value: E, unit: 'V' }, R: { value: R, unit: 'Ohm' }, tau: { value: tau, unit: 's' } } },
        ],
      },
      errors: [],
      warnings,
