  ...
  private buildResult(
    ...
    return {
      ...
      explanation: {
        ...
        steps: [
          { order: 1, description: '洛伦兹力', formula: 'F = qv × B', calculation: `F = ${q} × ${Vec2.magnitude(v0).toFixed(2)} × ${Bz} = ${(Math.abs(q) * Vec2.magnitude(v0) * Math.abs(Bz)).toExponential(2)} N` },
          { order: 2, description: '向心力等于洛伦兹力', formula: 'mv²/R = qvB' },
          { order: 3, description: '回旋半径', formula: 'R = mv/(|q|B)', calculation: `R = ${R.toFixed(4)} m` },
          { order: 4, description: '回旋周期', formula: 'T = 2πm/(|q|B)', calculation: `T = ${T_period.toFixed(4)} s` },
        ],
        formulas: [
          { name: '洛伦兹力', formula: 'F = qvB', variables: { q: { value: q, unit: 'C' }, v: { value: Vec2.magnitude(v0), unit: 'm/s' }, B: { value: Math.abs(Bz), unit: 'T' } } },
          { name: '回旋半径', formula: 'R = mv/(|q|B)', variables: { R: { value: R, unit: 'm' }, m: { value: m, unit: 'kg' }, v: { value: Vec2.magnitude(v0), unit: 'm/s' } } },
          { name: '回旋周期', formula: 'T = 2πm/(|q|B)', variables: { T: { value: T_period, unit: 's' }, m: { value: m, unit: 'kg' } } },
        ],
      },
      errors: [],
      warnings: [],
