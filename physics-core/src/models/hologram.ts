...
export class HologramModel extends PhysicsModelBase {
    ...
    readonly errorSources = ['实际干板非线性响应', '全息图乳胶收缩', '再现像共轭光干扰'];
    readonly requiredParameters: ParameterSpec[] = [
        ...
        {
            name: 'referenceAmp',
            description: '参考光振幅 A_r (相对值 0-10)',
            unit: '',
            required: true,
            min: 0.1,
            max: 10
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        for (let i = 0; i <= N; i++) {
            ...
            const I_total =
                (Ao * Ao) / (orderPlus + 0.1) + (Ar * Ar) / (orderZero + 0.1) + (Ao * Ao) / (orderMinus + 0.1);
        ...
        return {
            ...
            explanation: {
                ...
                formulas: [
                    {
                        name: '记录光强',
                        formula: 'I = A_r^2 + A_o^2 + 2*A_r*A_o*cos(delta_phi)',
                        variables: { Ar: { value: Ar, unit: '' }, Ao: { value: Ao, unit: '' } }
                    },
                    {
                        name: '条纹间距',
