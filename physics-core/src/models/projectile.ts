...
export class ProjectileModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    // 图表
    const x_t: ChartSeries = { xLabel: '时间', yLabel: '水平位移', xUnit: 's', yUnit: 'm', points: trajectory.map(p => ({ x: p.t, y: p.position.x })) };
    const y_t: ChartSeries = { xLabel: '时间', yLabel: '竖直位移', xUnit: 's', yUnit: 'm', points: trajectory.map(p => ({ x: p.t, y: p.position.y })) };
    const v_t: ChartSeries = { xLabel: '时间', yLabel: '速度大小', xUnit: 's', yUnit: 'm/s', points: trajectory.map(p => ({ x: p.t, y: Math.sqrt(p.velocity.x * p.velocity.x + p.velocity.y * p.velocity.y) })) };
    const vx_t: ChartSeries = { xLabel: '时间', yLabel: '水平分速度', xUnit: 's', yUnit: 'm/s', points: trajectory.map(p => ({ x: p.t, y: p.velocity.x })) };
    const vy_t: ChartSeries = { xLabel: '时间', yLabel: '竖直分速度', xUnit: 's', yUnit: 'm/s', points: trajectory.map(p => ({ x: p.t, y: p.velocity.y })) };
    ...
    const steps: ExplanationStep[] = [
      { order: 1, description: '运动分解', formula: '水平:匀速; 竖直:自由落体', calculation: `v₀x = ${v0x.toFixed(2)} m/s (恒定), v₀y = ${v0y.toFixed(2)} m/s` },
      { order: 2, description: '最高点', formula: 'H = h₀ + v₀y² / (2g)', calculation: `H = ${h0.toFixed(2)} + ${v0y.toFixed(2)}² / (2×${g})`, result: `H = ${apexHeight.toFixed(3)} m` },
      { order: 3, description: '飞行时间', formula: 't = (v₀y + √(v₀y² + 2gh₀)) / g', calculation: `t = (${v0y.toFixed(2)} + √${disc.toFixed(2)}) / ${g}`, result: `t = ${tLand.toFixed(3)} s` },
      { order: 4, description: '射程', formula: 'R = v₀x · t', calculation: `R = ${v0x.toFixed(2)} × ${tLand.toFixed(3)}`, result: `R = ${range.toFixed(3)} m` },
    ];
    const formulas: FormulaUsage[] = [
      { name: '水平位移', formula: 'x = v₀·cosθ·t', variables: { 'v₀x': { value: v0x, unit: 'm/s' } } },
      { name: '竖直位移', formula: 'y = h₀ + v₀·sinθ·t − ½gt²', variables: { 'v₀y': { value: v0y, unit: 'm/s' }, g: { value: g, unit: 'm/s²' } } },
      { name: '射程', formula: 'R = v₀²·sin(2θ)/g (h₀=0 时)', variables: { R: { value: range, unit: 'm' } } },
      { name: '最大高度', formula: 'H = h₀ + v₀²·sin²θ / (2g)', variables: { H: { value: apexHeight, unit: 'm' } } },
    ];

    return {
      meta: { model: 'projectile', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
