...
export class WaterDiffractionModel extends PhysicsModelBase {
    ...
    readonly errorSources = ['实际水波有衰减, 非线性效应', '近场 (菲涅耳) 衍射公式更复杂', '水槽边界反射干扰'];
        ...
        return {
            ...
            explanation: {
                ...
                formulas: [
                    {
                        name: '衍射强度',
                        formula: 'I(theta) = (sin(alpha)/alpha)^2, alpha = pi*a*sin(theta)/lambda',
                        variables: { a: { value: a, unit: 'cm' }, lambda: { value: lambda, unit: 'cm' } }
                    },
                    {
                        name: '半宽度',
