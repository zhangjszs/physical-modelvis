...
export class RadiationDeflectionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const B = c.Bfield, E_MeV = c.particleEnergy, type = c.particleType;
    const e = 1.602e-19, MeV_to_J = 1.602e-13;

    let m: number, q: number, label: string;
    if (type === 'alpha') { m = 4 * 1.66e-27; q = 2 * e; label = 'α'; }
    else if (type === 'beta') { m = 9.109e-31; q = -e; label = 'β'; }
    else { m = 1; q = 0; label = 'γ'; }
    ...
    return {
      meta: { model: 'radiation-deflection', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '入射', t: 0, position: { x: 0, y: 0 }, velocity: { x: v, y: 0 }, description: `${label} 粒子, E=${E_MeV}MeV, B=${B}T` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { B, E_MeV, v, r }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `${label} 射线: E=${E_MeV}MeV, B=${B}T, r=${r.toFixed(4)}m`, steps: [
        { order: 1, description: '洛伦兹力', formula: 'qvB = mv²/r', result: `r = mv/(qB) = ${r.toFixed(4)} m` },
        { order: 2, description: '三种射线对比', result: 'α 偏转半径 > β; γ 不偏转' },
      ], formulas: [{ name: '回旋半径', formula: 'r = mv/(qB)', variables: { m: { value: m, unit: 'kg' }, v: { value: v, unit: 'm/s' }, r: { value: r, unit: 'm' } } }] },
      errors: [], warnings: [],
    };
  }
}
