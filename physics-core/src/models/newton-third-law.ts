...
import type { SimulationResult, TrajectoryPoint, Keyframe, ChartSeries, ForceDiagram, ConservedQuantity } from '../types/result.js';
...
export class NewtonThirdLawModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const F_t: ChartSeries = {
      xLabel: '时间', yLabel: '力', xUnit: 's', yUnit: 'N',
    ...
    return {
      ...
      explanation: {
        ...
        steps: [
          { order: 1, description: '作用力与反作用力大小', formula: '|F_AB| = |F_BA|', calculation: `|F_AB|=${Math.abs(F_AB)}N, |F_BA|=${Math.abs(F_BA)}N` },
          { order: 2, description: '方向关系', formula: 'F_AB = -F_BA', result: '方向相反，沿同一直线' },
          { order: 3, description: '作用对象', formula: 'F_AB 作用在 B 上，F_BA 作用在 A 上', result: '不能抵消 (不同于平衡力)' },
          { order: 4, description: allowMotion ? '系统加速度' : '静止条件', formula: allowMotion ? 'a = F/(mA+mB)' : 'ΣF_A = 0', calculation: allowMotion ? `a = ${F_AB}/(${mA}+${mB}) = ${aSystem.toFixed(3)} m/s²` : '外力为零，两物体保持静止' },
        ],
        formulas: [
          { name: '牛顿第三定律', formula: 'F_AB = -F_BA', variables: { F_AB: { value: F_AB, unit: 'N' }, F_BA: { value: F_BA, unit: 'N' } } },
          { name: '大小相等', formula: '|F_AB| = |F_BA|', variables: { '|F_AB|': { value: Math.abs(F_AB), unit: 'N' }, '|F_BA|': { value: Math.abs(F_BA), unit: 'N' } } },
