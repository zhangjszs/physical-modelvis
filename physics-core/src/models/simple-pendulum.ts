...
export class SimplePendulumModel extends PhysicsModelBase {
    ...
    readonly assumptions = ['摆线轻质不可伸长', '摆球视为质点', '忽略空气阻力', '悬点固定'];
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        if (
            Math.abs(body.position.x - expectedX) > posTolerance ||
            Math.abs(body.position.y - expectedY) > posTolerance
        ) {
        ...
        const theta_t: ChartSeries = {
            ...
            points: trajectory.map((p, i) => ({
                x: p.t,
                y: (this.thetaFromPosition(p.position, pivot, L) * 180) / Math.PI
            }))
        };
                ...
                ? [
                      {
                          ...
                          law: '机械能守恒 (无阻尼)',
                          initialValue: E0,
                          finalValue: this.computeEnergy(
                              mass,
                              L,
                              g,
                              this.thetaFromPosition(last.position, pivot, L),
                              (last.velocity.x * Math.cos(this.thetaFromPosition(last.position, pivot, L)) -
                                  last.velocity.y * Math.sin(this.thetaFromPosition(last.position, pivot, L))) /
                                  L,
                              pivot
                          ),
        ...
        const steps: ExplanationStep[] = [
            {
                ...
                description: '小角度周期公式',
                formula: 'T = 2π√(L/g)',
                calculation: `T = 2π√(${L}/${g}) = ${Tsmall.toFixed(3)} s`
            },
            {
                order: 2,
                description: '角频率',
                formula: 'ω = √(g/L)',
                calculation: `ω = √(${g}/${L}) = ${omegaSmall.toFixed(3)} rad/s`
            },
            {
                order: 3,
                description: '最大回复力 (小角度)',
                formula: 'F_max = mg·sinθ₀',
                calculation: `F_max = ${mass}×${g}×sin${deg0}° = ${(mass * g * Math.sin(theta0Rad)).toFixed(2)} N`
            },
            {
                order: 4,
                description: '能量转换',
                formula: 'E = ½mv² + mgh = const',
                result: damping > 0 ? '阻尼消耗能量，振幅递减' : '无阻尼: 机械能守恒'
            }
        ];
                ...
                formulas: [
                    {
                        ...
                        variables: {
                            L: { value: L, unit: 'm' },
                            g: { value: g, unit: 'm/s²' },
                            T: { value: Tsmall, unit: 's' }
                        }
                    },
                    {
                        name: '回复力',
                        formula: 'F = −mg·sinθ ≈ −mgθ (小角度)',
                        variables: { m: { value: mass, unit: 'kg' }, g: { value: g, unit: 'm/s²' } }
                    },
