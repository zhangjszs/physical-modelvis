...
export class MomentumModel extends PhysicsModelBase {
    ...
    readonly assumptions = ['系统不受外力 (或外力远小于内力)', '一维运动 (沿 x 轴)', '碰撞/作用瞬间完成'];
    readonly applicableRange = '动量定理 (恒力冲量)、反冲运动 (火箭、喷气)';
    readonly errorSources = ['实际外力 (摩擦、重力) 可能影响动量守恒', '喷出物速度分布非均匀'];
    ...
    private solveImpulse(
        problem: PhysicsProblem,
        mc: NonNullable<PhysicsProblem['constraints']>['momentum'] & object
    ): SimulationResult {
        ...
        const keyframes: Keyframe[] = [
            {
                ...
                velocity: { x: v0, y: 0 },
                description: `v₀=${v0.toFixed(2)}m/s, p₀=${pInit.toFixed(2)}kg·m/s`
            },
            {
                label: '终点',
                t: duration,
                position: trajectory[trajectory.length - 1].position,
                velocity: { x: v0 + (F * duration) / m, y: 0 },
                description: `v=${(v0 + (F * duration) / m).toFixed(2)}m/s, Δp=${deltaP.toFixed(2)}kg·m/s`
            }
        ];
        ...
        const p_t: ChartSeries = {
            ...
            xUnit: 's',
            yUnit: 'kg·m/s',
            points: trajectory.map(p => ({ x: p.t, y: m * p.velocity.x }))
        };
                ...
                steps: [
                    {
                        ...
                        description: '冲量定义',
                        formula: 'J = F·Δt',
                        calculation: `J = ${F} × ${duration} = ${impulse.toFixed(2)} N·s`
                    },
                    {
                        order: 2,
                        description: '动量定理',
                        formula: 'J = Δp = m·Δv',
                        calculation: `${impulse.toFixed(2)} = ${m} × (${(v0 + (F * duration) / m).toFixed(2)} - ${v0})`
                    },
                    {
                        order: 3,
                        description: '末速度',
                        formula: 'v = v₀ + F·t/m',
                        calculation: `v = ${v0} + ${impulse.toFixed(2)}/${m} = ${(v0 + (F * duration) / m).toFixed(2)} m/s`
                    }
                ],
                formulas: [
                    {
    ...
    private solveRecoil(
        problem: PhysicsProblem,
        _mc: NonNullable<PhysicsProblem['constraints']>['momentum'] & object
    ): SimulationResult {
        ...
        const keyframes: Keyframe[] = [
            {
                label: '初始静止',
                t: 0,
                position: { x: x1Init, y: 0 },
                velocity: { x: 0, y: 0 },
                description: `两物体静止: m₁=${m1}kg, m₂=${m2}kg`
            },
            {
                label: '分离后',
                t: duration,
                position: { x: x1Init + v1Final * duration, y: 0 },
                velocity: { x: v1Final, y: 0 },
                description: `v₁=${v1Final.toFixed(3)}m/s, v₂=${v2Final.toFixed(3)}m/s, 总动量=0`
            }
        ];
        ...
        const v2_t: ChartSeries = {
            xLabel: '时间',
            yLabel: '物体2速度',
            xUnit: 's',
            yUnit: 'm/s',
            points: traj2.map(p => ({ x: p.t, y: p.velocity.x }))
        };
                ...
                steps: [
                    {
                        order: 1,
                        description: '动量守恒',
                        formula: 'm₁v₁ + m₂v₂ = 0 (初始静止)',
                        calculation: `${m1}×(${v1Final.toFixed(3)}) + ${m2}×${v2Final.toFixed(3)} = 0`
                    },
                    {
                        order: 2,
                        description: '速度比',
                        formula: 'v₁/v₂ = −m₂/m₁',
                        calculation: `${v1Final.toFixed(3)}/${v2Final.toFixed(3)} = ${(-m2 / m1).toFixed(3)}`
                    },
