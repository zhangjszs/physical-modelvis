...
export class SurfaceTensionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const forceCurve: ChartSeries = {
      xLabel: '滑块长度 L (cm)', yLabel: '表面张力 F_sigma (mN)', xUnit: 'cm', yUnit: 'mN',
      points: [],
    };
    const nScan = 50;
    ...
    const sigmaCurve: ChartSeries = {
      xLabel: '温度 T (°C)', yLabel: '表面张力系数 sigma (N/m)', xUnit: '°C', yUnit: 'N/m',
      points: [],
    };
    const nT = 50;
    for (let i = 0; i <= nT; i++) {
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '温度依赖', formula: 'sigma(T)=sigma_0-beta(T-T_0)', variables: { sigma_0: { value: sigma0, unit: 'N/m' }, beta: { value: beta, unit: 'N/(m·K)' }, T: { value: T, unit: '°C' } } },
          { name: '表面张力', formula: 'F_sigma=2·sigma·L', variables: { sigma: { value: sigma, unit: 'N/m' }, L: { value: L, unit: 'm' } } },
        ],
      },
      errors: [],
      warnings,
