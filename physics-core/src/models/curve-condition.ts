...
export class CurveConditionModel extends PhysicsModelBase {
    ...
    readonly requiredParameters: ParameterSpec[] = [
        {
            name: 'forceDirectionDeg',
            description: '合力方向角 (°, 相对 +x)',
            unit: '°',
            required: true,
            min: -180,
            max: 360
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        const formulas: FormulaUsage[] = [
            {
                name: '轨迹方程',
                formula: 'x(t)=v₀·t+½·ax·t², y(t)=½·ay·t²',
                variables: {
                    v0: { value: v0Mag, unit: 'm/s' },
                    ax: { value: ax, unit: 'm/s²' },
                    ay: { value: ay, unit: 'm/s²' }
                }
            },
            { name: '曲线条件', formula: 'F⃗∥v⃗₀ → 直线,  F⃗∦v⃗₀ → 曲线', variables: {} },
            {
                name: '牛顿第二定律',
                formula: 'F⃗ = m·a⃗',
                variables: { F: { value: F, unit: 'N' }, m: { value: m, unit: 'kg' } }
            }
        ];
