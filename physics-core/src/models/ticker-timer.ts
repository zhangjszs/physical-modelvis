...
export class TickerTimerModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    if (absError > 1e-9) {
      warnings.push(`逐差法 â (${aDiscrete.toExponential(2)}) 与标称 a (${a.toExponential(2)}) 偏差超过浮点精度, 请检查数值`);
    }
    ...
    const steps: ExplanationStep[] = [
      ...
      },
      {
        ...
        calculation: `s₁~s₆ = [${s.slice(1).map(v => (v * 100).toFixed(2)).join(', ')}] cm` +
          ` → â = ${aDiscrete.toFixed(4)} m/s²`,
