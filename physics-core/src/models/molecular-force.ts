...
export class MolecularForceModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const potentialCurve: ChartSeries = {
      xLabel: '分子间距 r (nm)', yLabel: '势能 U (10^-21 J)', xUnit: 'nm', yUnit: '10^-21 J',
      points: [],
    };
    // 力函数 F(r) (正=斥力, 负=引力)
    const forceCurve: ChartSeries = {
      xLabel: '分子间距 r (nm)', yLabel: '作用力 F (10^-12 N)', xUnit: 'nm', yUnit: '10^-12 N',
      points: [],
    };
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: 'LJ 势', formula: 'U(r)=4ε[(σ/r)^12-(σ/r)^6]', variables: { epsilon: { value: eps, unit: 'J' }, sigma: { value: sigma, unit: 'm' } } },
          { name: '平衡位置', formula: 'r_eq=2^(1/6)·σ', variables: { sigma: { value: sigma, unit: 'm' }, rEq: { value: rEq, unit: 'm' } } },
          { name: '分子力', formula: 'F(r)=(24ε/r)·[2(σ/r)^12-(σ/r)^6]', variables: { epsilon: { value: eps, unit: 'J' } } },
        ],
      },
      errors: [],
      warnings,
