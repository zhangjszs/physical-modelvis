...
export class RefractionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const sinRelation: ChartSeries = {
      xLabel: 'sinθ₁', yLabel: 'sinθ₂', xUnit: '', yUnit: '',
      points: [],
    };
    ...
    const angleRelation: ChartSeries = {
      xLabel: '入射角 θ₁ (°)', yLabel: '折射角 θ₂ (°)', xUnit: '°', yUnit: '°',
      points: [],
    };
    for (let deg = 0; deg <= 80; deg += 5) {
      const t1 = deg * Math.PI / 180;
    ...
    const keyframes: Keyframe[] = [{
      label: totalInternalReflection ? '全反射' : '折射', t: 0,
    ...
    const trajectory: TrajectoryPoint[] = [
      { t: 0, position: { x: incidentStartX, y: incidentStartY }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 },
      { t: 0, position: { x: incidentEndX, y: incidentEndY }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 },
    ];
    if (!totalInternalReflection) {
      trajectory.push({ t: 0, position: { x: refractEndX, y: refractEndY }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 });
    }
    trajectory.push({ t: 0, position: { x: reflectEndX, y: reflectEndY }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 });
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: 'Snell 定律', formula: 'n₁sinθ₁=n₂sinθ₂', variables: { n1: { value: n1, unit: '' }, n2: { value: n2, unit: '' }, 'θ₁': { value: theta1Deg, unit: '°' }, 'θ₂': { value: isNaN(theta2Deg) ? 0 : theta2Deg, unit: '°' } } },
          { name: '临界角', formula: 'θ_c = arcsin(n₂/n₁) (n₁>n₂)', variables: { n1: { value: n1, unit: '' }, n2: { value: n2, unit: '' } } },
        ],
      },
      errors: [],
      warnings,
