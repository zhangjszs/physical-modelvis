...
export class RadioactiveDecayModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const N_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '剩余原子数 N(t)', xUnit: 's', yUnit: '个',
      points: [],
    };
    const A_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '活度 A (Bq)', xUnit: 's', yUnit: 'Bq',
      points: [],
    };
    const N = 200;
    for (let i = 0; i <= N; i++) {
    ...
    for (let k = 0; k < nToShow; k++) {
      ...
      const r2 = Math.abs(Math.sin(seed * 78.233) * 43758.5453) % 1;
      ...
      trajectories.push({ t: 0, position: { x, y }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 }); // 起点
      ...
      for (let s = 0; s < L; s++) {
        ...
        trajectories.push({ t: s, position: { x, y }, velocity: { x: stepLen * Math.cos(theta), y: stepLen * Math.sin(theta) }, kineticEnergy: 0, potentialEnergy: 0 });
      }
    }
    ...
    // 射线类型说明
    const rayTypeZh = radiationType === 'alpha' ? 'α 粒子 (He 核, 电离强)' : radiationType === 'beta' ? 'β 粒子 (电子, 电离中)' : 'γ 光子 (电离弱)';
    steps.push({
      order: 4,
      description: '射线类型',
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '衰变定律', formula: 'N=N₀e^(−λt)', variables: { N0: { value: N0, unit: '个' }, λ: { value: lambda, unit: 's⁻¹' }, 'T₁/₂': { value: halfLife, unit: 's' } } },
