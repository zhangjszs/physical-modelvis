...
export class AmpereForceModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const F_I_curve: ChartSeries = {
      xLabel: '电流 I (A)', yLabel: '安培力 F (N)', xUnit: 'A', yUnit: 'N',
    ...
    const F_L_curve: ChartSeries = {
      xLabel: '导线长度 L (m)', yLabel: '安培力 F (N)', xUnit: 'm', yUnit: 'N',
      points: Array.from({ length: 100 }, (_, i) => {
    ...
    const F_sinTheta_curve: ChartSeries = {
      xLabel: 'sinθ', yLabel: '安培力 F (N)', xUnit: '', yUnit: 'N',
    ...
    return {
      meta: { model: 'ampere-force', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '安培力', formula: 'F=BIL·sinθ', variables: { F: { value: F, unit: 'N' }, B: { value: B, unit: 'T' }, I: { value: I, unit: 'A' }, L: { value: L, unit: 'm' }, 'sinθ': { value: Math.sin(angleRad), unit: '' } } },
          { name: 'F-I 斜率', formula: 'dF/dI = BL·sinθ', variables: { '斜率': { value: B * L * Math.sin(angleRad), unit: 'N/A' } } },
          { name: 'F-L 斜率', formula: 'dF/dL = BI·sinθ', variables: { '斜率': { value: B * I * Math.sin(angleRad), unit: 'N/m' } } },
        ],
      },
      errors: [],
      warnings,
