    ...
    return {
      ...
      explanation: {
        ...
        steps: [
          ...
          { order: 4, description: '周期', formula: 'T = 2π/ω', calculation: `T = 2π/${omega.toFixed(2)} = ${period.toFixed(3)} s` },
          { order: 5, description: '转速（频率）', formula: 'f = 1/T = ω/(2π)', calculation: `f = ${frequency.toFixed(3)} Hz (转/秒)` },
        ],
        formulas: [
          { name: '线速度-角速度关系', formula: 'v = ωr', variables: { v: { value: v, unit: 'm/s' }, 'ω': { value: omega, unit: 'rad/s' }, r: { value: radius, unit: 'm' } } },
          { name: '向心加速度', formula: 'a = ω²r = v²/r', variables: { a: { value: aCentripetal, unit: 'm/s²' }, 'ω': { value: omega, unit: 'rad/s' }, r: { value: radius, unit: 'm' } } },
          { name: '向心力公式', formula: 'F = mω²r = mv²/r', variables: { F: { value: F, unit: 'N' }, m: { value: mass, unit: 'kg' }, 'ω': { value: omega, unit: 'rad/s' }, r: { value: radius, unit: 'm' } } },
          { name: '周期公式', formula: 'T = 2π/ω = 2πr/v', variables: { T: { value: period, unit: 's' }, 'ω': { value: omega, unit: 'rad/s' } } },
        ],
      },
      errors: [],
      warnings: [],
