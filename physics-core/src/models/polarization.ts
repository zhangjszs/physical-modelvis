...
export class PolarizationModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    { name: 'initialIntensity', description: '入射光强 I_0 (相对值 0-1)', unit: '', required: true, min: 0, max: 1 },
    { name: 'nPolarizers', description: '偏振片数量', unit: '', required: true, min: 1, max: 5 },
    { name: 'polarizerAngles', description: '各偏振片透振方向角度数组 (度), 长度=nPolarizers', unit: 'deg', required: true },
    { name: 'incidentAngle', description: '入射光偏振方向 (度, 仅 nPolarizers>=1 时有效)', unit: 'deg', required: false, min: 0, max: 360 },
  ];
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '马吕斯定律', formula: 'I = I_0*cos^2(theta)', variables: { I0: { value: I0, unit: '' }, I: { value: Ifinal, unit: '' } } },
          { name: '多片系统', formula: 'I_n = I_0 * prod(cos^2(delta_theta_i))', variables: { n: { value: n, unit: '' } } },
        ],
      },
      errors: [],
      warnings,
