...
export class DiffractionGratingModel extends PhysicsModelBase {
    ...
    readonly assumptions = ['大量平行等距狭缝 (N >> 1)', '垂直入射, 平行单色光', '远场条件 (L >> d)'];
    readonly applicableRange = 'gratingConstant: 1--10 um; wavelength: 380--780 nm; orderMax: 0--5';
    readonly errorSources = ['实际光栅缺陷, 鬼线', '高级次谱线强度弱', '缝宽 a 接近 d 时缺级'];
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        const N_sample = 800;
        const thetaMax =
            (Math.max(30, principalMaxima.length > 0 ? principalMaxima[principalMaxima.length - 1]!.thetaDeg + 5 : 30) *
                Math.PI) /
            180;
            ...
            },
            {
                ...
                result:
                    missingOrders.length > 0
                        ? `缺级 k = ${missingOrders.join(', ')} (ratio d/a = ${ratio.toFixed(2)})`
                        : '无缺级'
            },
            {
                order: 4,
            ...
            explanation: {
                ...
                formulas: [
                    ...
                    },
                    {
                        name: '缺级条件',
                        formula: 'k = m*(d/a)',
                        variables: { 'd/a': { value: parseFloat(ratio.toFixed(3)), unit: '' } }
                    }
                ]
            },
            errors: [],
