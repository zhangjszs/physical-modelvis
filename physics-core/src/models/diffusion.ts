...
export class DiffusionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const D = dc.diffusionCoeff ?? (mode === 'gas'
      ? 1e-5 * Math.pow(T / 300, 1.5)
      : 1e-9 * Math.pow(T / 300, 1.5));
    ...
    const concentrationProfile: ChartSeries = {
      xLabel: '位置 x (μm)', yLabel: '浓度 C (a.u.)', xUnit: 'μm', yUnit: 'a.u.',
      points: [],
    };
    const Nx = 100;
    for (let i = 0; i <= Nx; i++) {
    ...
    const msdCurve: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '均方位移 MSD (μm²)', xUnit: 's', yUnit: 'μm²',
      points: [],
    };
    ...
    const keyframes: Keyframe[] = [
      ...
      },
      {
        label: '扩散进行中',
        t: tSample,
        position: { x: parseFloat((gridSize * 1e6).toFixed(3)), y: parseFloat((N / Math.sqrt(4 * Math.PI * D * tSample)).toFixed(1)) },
        velocity: { x: 0, y: 0 },
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '均方位移', formula: 'MSD = 2Dt', variables: { D: { value: D, unit: 'm²/s' }, t: { value: tSample, unit: 's' }, MSD: { value: 2 * D * tSample, unit: 'm²' } } },
          { name: '高斯分布', formula: 'C(x,t) = (N/√(4πDt))·exp(-x²/(4Dt))', variables: { N: { value: N, unit: '个' }, D: { value: D, unit: 'm²/s' }, t: { value: tSample, unit: 's' } } },
        ],
      },
      errors: [],
      warnings,
