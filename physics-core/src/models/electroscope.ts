...
export class ElectroscopeModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    // 给出几个关键帧
    const sampleIndices = [0, Math.floor(sampleCount * 0.25), Math.floor(sampleCount * 0.5), Math.floor(sampleCount * 0.75), sampleCount];
    ...
    const q_theta: ChartSeries = {
      xLabel: '带电量 q (μC)', yLabel: '箔片张角 θ (度)', xUnit: 'μC', yUnit: '°',
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '库仑斥力', formula: 'F_k = k·q²/(4L²)', variables: { k: { value: K_FOIL, unit: 'N·m²/C²' }, q: { value: q_C, unit: 'C' }, L: { value: L_m, unit: 'm' } } },
          { name: '张角公式', formula: 'sin(θ) = k·q²/(4mgL)', variables: { q: { value: q_C, unit: 'C' }, m: { value: m_kg, unit: 'kg' } } },
        ],
      },
      renderHints: [
