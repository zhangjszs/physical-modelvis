  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (let s = 0; s <= sampleCount; s++) {
      ...
      for (let i = 0; i < N; i++) {
        ...
        const pos = mode === 'longitudinal'
          ? { x: xEq + y, y: 0 }
          : { x: xEq, y: -y }; // 屏幕 y 向下, 物理位移 y 为正则屏幕 y 减小
        // 速度 (解析微分)
    ...
    const wave_t: ChartSeries = {
      xLabel: '位置 x', yLabel: '位移 y', xUnit: 'm', yUnit: 'm',
      points: waveSnapshot.map(p => ({ x: p.position.x, y: p.position.y })),
    };
    // 特定质点的 y-t 振动图 (取中间质点)
    const midIdx = Math.floor(trajs.length / 2);
    const y_t: ChartSeries = {
      xLabel: '时间', yLabel: '质点位移', xUnit: 's', yUnit: 'm',
      points: trajs[midIdx]!.map(p => ({ x: p.t, y: p.position.y })),
    };
    // 波速-波长-频率关系 (常量)
    const v_t: ChartSeries = {
      xLabel: '时间', yLabel: '波速 v', xUnit: 's', yUnit: 'm/s',
      points: trajs[0]!.map(p => ({ x: p.t, y: waveSpeed })),
    };

    const steps: ExplanationStep[] = [
      { order: 1, description: '波速公式', formula: 'v = f·λ', calculation: `v = ${f.toFixed(2)} × ${lambda.toFixed(2)} = ${waveSpeed.toFixed(2)} m/s` },
      { order: 2, description: '波数 k', formula: 'k = 2π/λ', calculation: `k = 2π/${lambda.toFixed(2)} = ${k.toFixed(2)} rad/m` },
      { order: 3, description: '角频率 ω', formula: 'ω = 2πf', calculation: `ω = 2π×${f.toFixed(2)} = ${omega.toFixed(2)} rad/s` },
      { order: 4, description: mode === 'interference'
        ? '干涉叠加'
        : (mode === 'longitudinal' ? '纵波: 振动方向与传播方向平行' : '横波: 振动方向与传播方向垂直'),
        formula: mode === 'interference'
          ? 'y = y₁ + y₂ = A₁sin(ωt−kx) + A₂sin(ωt+kx+φ)'
          : 'y = A·sin(ωt − kx)',
      },
    ];

    return {
      meta: { model: 'mechanical-wave', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '波速公式', formula: 'v = f·λ', variables: { v: { value: waveSpeed, unit: 'm/s' }, f: { value: f, unit: 'Hz' }, λ: { value: lambda, unit: 'm' } } },
          { name: '波形方程', formula: 'y = A·sin(ωt − kx)', variables: { A: { value: A, unit: 'm' }, ω: { value: omega, unit: 'rad/s' }, k: { value: k, unit: 'rad/m' } } },
        ],
      },
      errors: [],
      warnings: [],
