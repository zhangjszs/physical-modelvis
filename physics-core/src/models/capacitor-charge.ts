...
export class CapacitorChargeModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const Uc_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '电容电压 U_c (V)', xUnit: 's', yUnit: 'V',
      points: [],
    };
    // 图表 2: I-t 指数衰减
    const I_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '电流 I (A)', xUnit: 's', yUnit: 'A',
      points: [],
    };
    // 图表 3: Q-t (充电累积) 或 ln(U_c)-t (放电直线)
    const Q_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: '电荷 Q (C)', xUnit: 's', yUnit: 'C',
      points: [],
    };
    const lnUc_t: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: 'ln(U_c) (V)', xUnit: 's', yUnit: '',
      points: [],
    };
    ...
    for (const k of [1, 2, 3, 4, 5]) {
      ...
      const pct = mode === 'charge'
        ? ((1 - expTerm) * 100).toFixed(1)
        : ((expTerm) * 100).toFixed(1);
      keyframes.push({
        label: `${k}τ`,
        t,
        position: { x: 0, y: 0 },
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '时间常数', formula: 'τ = RC', variables: { R: { value: R, unit: 'Ω' }, C: { value: C, unit: 'F' }, τ: { value: tau, unit: 's' } } },
          { name: mode === 'charge' ? '充电电压' : '放电电压', formula: mode === 'charge' ? 'U_c = E(1−e^(−t/τ))' : 'U_c = E·e^(−t/τ)', variables: { E: { value: E, unit: 'V' } } },
          { name: '电流', formula: 'I = (E/R)·e^(−t/τ)', variables: { E: { value: E, unit: 'V' }, R: { value: R, unit: 'Ω' } } },
        ],
      },
      errors: [],
      warnings,
