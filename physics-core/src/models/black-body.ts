            ...
            keyframes: [
                {
                    label: '峰值',
                    t: 0,
                    position: { x: cLight / lambdaPeak, y: maxI },
                    velocity: { x: 0, y: 0 },
                    description: `λ_peak=${(lambdaPeak * 1e9).toFixed(0)}nm, T=${T}K`
                }
            ],
            charts: { x_t },
            diagnostics: {
                conservedQuantities: [],
                maxValues: { T, lambdaPeak: lambdaPeak * 1e6, totalPower: totalPower / 1000, maxI },
                rangeCheck: { withinRange: true, warnings: [] }
            },
            explanation: {
                summary: `黑体 T=${T}K, λ_peak=${(lambdaPeak * 1e6).toFixed(0)}μm, E_total=${(totalPower / 1000).toFixed(1)} kW/m²`,
                steps: [
                    {
                        order: 1,
                        description: '维恩位移律',
                        formula: 'λ_peak · T = b = 2.898×10⁻³ m·K',
                        result: `λ_peak = ${(lambdaPeak * 1e6).toFixed(1)} μm`
                    },
                    {
                        order: 2,
                        description: '斯特藩-玻尔兹曼',
                        formula: 'E = σ·T⁴',
                        result: `E = ${(totalPower / 1000).toFixed(1)} kW/m²`
                    }
                ],
                formulas: [
                    {
                        name: '维恩',
                        formula: 'λ_peak = b/T',
                        variables: { T: { value: T, unit: 'K' }, lambda_peak: { value: lambdaPeak * 1e6, unit: 'μm' } }
                    }
                ]
            },
            errors: [],
