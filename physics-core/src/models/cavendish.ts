...
export class CavendishModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'torsionConst', description: '扭转常数 k (N·m/rad)', unit: 'N·m/rad', required: true, min: 1e-15,max: 1 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const c = problem.constraints?.cavendish
      ?? { m1: 10, m2: 0.5, distance: 0.1, torsionConst: 1e-4, mirrorDist: 5, armLength: 1 };
    ...
    const magnificationLinear = deltaSpotLinear > 0 && F > 0 ? deltaSpotLinear / F : 0;
    ...
    const steps = this.buildSteps(m1, m2, r, k, D, L, F, theta, thetaDeg, deltaSpotLinear, deltaSpotExact, G_fit, relativeError, amplification, torque);
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '万有引力', formula: 'F = G·m₁·m₂/r²',       variables: { G: { value: G_STANDARD, unit: 'N·m²/kg²' }, m1: { value: m1, unit: 'kg' }, m2: { value: m2, unit: 'kg' }, r: { value: r, unit: 'm' } } },
          { name: '力矩放大', formula: 'τ = F·L',              variables: { F: { value: F, unit: 'N' }, L: { value: L, unit: 'm' } } },
          { name: '扭转放大', formula: 'θ = τ/k',              variables: { τ: { value: torque, unit: 'N·m' }, k: { value: k, unit: 'N·m/rad' } } },
          { name: '光杠杆放大', formula: 'Δspot = 2·D·θ ≈ 4D·θ (小角)', variables: { D: { value: D, unit: 'm' }, θ: { value: theta, unit: 'rad' } } },
          { name: '拟合 G', formula: 'G = θ·k·r² / (m₁·m₂·L)', variables: { G_fit: { value: G_fit, unit: 'N·m²/kg²' } } },
        ],
      },
      renderHints: [
  ...
  private buildSteps(
    m1: number, m2: number, r: number, k: number, D: number, L: number,
    F: number, theta: number, thetaDeg: number,
    deltaSpotLinear: number, deltaSpotExact: number,
    G_fit: number, relErr: number, amplification: number, torque: number,
  ) {
