...
export class LoadVoltageModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const U_R: ChartSeries = {
      xLabel: '负载电阻 R (Ω)', yLabel: '路端电压 U (V)', xUnit: 'Ω', yUnit: 'V',
      points: [],
    };
    // 图表 2: U-I 直线 (U = E − I·r) — 关键点: 严格线性
    const U_I: ChartSeries = {
      xLabel: '电流 I (A)', yLabel: '路端电压 U (V)', xUnit: 'A', yUnit: 'V',
      points: [],
    };
    // 图表 3: I-R 曲线 (I = E/(R+r))
    const I_R: ChartSeries = {
      xLabel: '负载电阻 R (Ω)', yLabel: '电流 I (A)', xUnit: 'Ω', yUnit: 'A',
      points: [],
    };
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '全电路欧姆定律', formula: 'I = E/(R+r)', variables: { E: { value: E, unit: 'V' }, R: { value: R0, unit: 'Ω' }, r: { value: r, unit: 'Ω' }, I: { value: I0, unit: 'A' } } },
          { name: '路端电压', formula: 'U = E·R/(R+r) = E−Ir', variables: { U: { value: U0, unit: 'V' } } },
          { name: 'U-I 线性', formula: 'U = E − Ir', variables: { E: { value: E_exp, unit: 'V' }, r: { value: r_exp, unit: 'Ω' } } },
        ],
      },
      errors: [],
      warnings,
