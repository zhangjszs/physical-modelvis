...
export class BrownianMotionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (let p = 0; p < nParticles; p++) {
      ...
      for (let i = 1; i <= nSteps; i += step) {
        ...
        traj.push({
          ...
          velocity: { x: parseFloat((sigma * xiX / (dt * step)).toFixed(6)), y: parseFloat((sigma * xiY / (dt * step)).toFixed(6)) },
          kineticEnergy: 0,
          potentialEnergy: 0,
        });
      }
    ...
    const xSeries: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: 'x 位置 (μm)', xUnit: 's', yUnit: 'μm',
    ...
    const ySeries: ChartSeries = {
      xLabel: '时间 t (s)', yLabel: 'y 位置 (μm)', xUnit: 's', yUnit: 'μm',
      points: trajectories[0].map(p => ({
        x: parseFloat(p.t.toFixed(4)),
    ...
    const keyframes: Keyframe[] = [
      ...
      },
      {
        label: '终点',
        t: duration,
        position: { x: parseFloat((finalPos.x * 1e6).toFixed(3)), y: parseFloat((finalPos.y * 1e6).toFixed(3)) },
        velocity: { x: 0, y: 0 },
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: 'Stokes-Einstein', formula: 'D = kT/(6πηr)', variables: { k: { value: kB, unit: 'J/K' }, T: { value: T, unit: 'K' }, eta: { value: eta, unit: 'Pa·s' }, r: { value: r, unit: 'm' } } },
          { name: '均方位移 2D', formula: 'MSD = 4Dt', variables: { D: { value: D, unit: 'm²/s' }, t: { value: duration, unit: 's' } } },
        ],
      },
      errors: [],
      warnings,
