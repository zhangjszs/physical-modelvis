    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        const steps: ExplanationStep[] = [
            ...
            },
            {
                order: 4,
                description: '结论',
                formula: mode === 'damping' ? '5*tau_c 后 omega < 1% omega_0' : '5*tau_c 后 omega > 99% omega_0',
                result:
                    mode === 'damping'
                ...
                formulas: [
                    {
                        ...
                        formula: 'tau_c = J/(k*B^2)',
                        variables: {
                            J: { value: J, unit: 'kg·m^2' },
                            k: { value: k, unit: '' },
                            B: { value: B, unit: 'T' },
                            tau_c: { value: tauC, unit: 's' }
                        }
                    },
                    {
                        name: mode === 'damping' ? '阻尼方程' : '驱动方程',
                        formula: mode === 'damping' ? 'omega=omega_0*exp(-t/tau_c)' : 'omega=omega_0*(1-exp(-t/tau_c))',
                        variables: { omega_0: { value: omega0, unit: 'rad/s' }, tau_c: { value: tauC, unit: 's' } }
                    }
                ]
            },
            errors: [],
