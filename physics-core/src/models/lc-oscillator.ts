...
export class LCOscillatorModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const q_t: ChartSeries = {
      xLabel: '时间 t (μs)', yLabel: '电荷 q (μC)', xUnit: 'μs', yUnit: 'μC',
      points: [],
    };
    const i_t: ChartSeries = {
      xLabel: '时间 t (μs)', yLabel: '电流 i (mA)', xUnit: 'μs', yUnit: 'mA',
      points: [],
    };
    const Ee_t: ChartSeries = {
      xLabel: '时间 t (μs)', yLabel: '电场能 (μJ)', xUnit: 'μs', yUnit: 'μJ',
      points: [],
    };
    const Em_t: ChartSeries = {
      xLabel: '时间 t (μs)', yLabel: '磁场能 (μJ)', xUnit: 'μs', yUnit: 'μJ',
      points: [],
    };
    ...
    const stepsExpl: ExplanationStep[] = [
      { order: 1, description: '振荡周期', formula: 'T = 2π√(LC)', calculation: `T = 2π√(${L}×${C}) = ${(T * 1e6).toFixed(2)} μs` },
      { order: 2, description: '振荡频率', formula: 'f = 1/(2π√(LC))', calculation: `f = ${(f / 1e3).toFixed(2)} kHz` },
      { order: 3, description: '电荷与电流', formula: 'q=Q₀·cos(ωt), i=−Q₀ω·sin(ωt)', result: `Q₀=${(Q0 * 1e6).toFixed(1)}μC, Iₘ=${(Im * 1e3).toFixed(2)}mA` },
      { order: 4, description: '能量守恒', formula: 'E_total = Q₀²/(2C) = 常数', calculation: `E_total = ${(E_total * 1e6).toFixed(3)} μJ (电场能↔磁场能)` },
    ];

    return {
      meta: { model: 'lc-oscillator', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps: stepsExpl,
        formulas: [
          { name: '振荡周期', formula: 'T=2π√(LC)', variables: { L: { value: L, unit: 'H' }, C: { value: C, unit: 'F' }, T: { value: T, unit: 's' } } },
          { name: '电荷', formula: 'q=Q₀·cosωt', variables: { Q0: { value: Q0, unit: 'C' }, ω: { value: omega, unit: 'rad/s' } } },
        ],
      },
      errors: [],
      warnings: [],
