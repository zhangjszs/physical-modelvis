...
export class OverweightModel extends PhysicsModelBase {
    ...
    readonly errorSources = ['实际电梯加速度非阶跃 (有平滑过渡)', '地球自转影响微小重力偏差', '测量仪器精度限制'];
        ...
        const formulas: FormulaUsage[] = [
            ...
            },
            {
                ...
                variables: {
                    N: { value: N, unit: 'N' },
                    mg: { value: m * g, unit: 'N' },
                    'm·a_y': { value: m * aY, unit: 'N' }
                }
            }
        ];

        const modeLabel = OverweightModel.phaseLabel(mode);
        const stateLabel = isOverweight
            ? '超重'
            : isWeightless
              ? '失重'
              : isCompleteWeightlessness
...
function withinRangeCheck(
    isCompleteWeightlessness: boolean,
    aMag: number,
    g: number
