...
export class ResonanceModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'springConstant', description: '弹簧劲度系数 k (N/m)', unit: 'N/m', required: true, min: 0.1, max: 1000 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const peakA = betaRef < omega0 / Math.sqrt(2) ? (F0 / m) / (2 * betaRef * Math.sqrt(omega0 * omega0 - betaRef * betaRef)) : (F0 / m) / (omega0 * omega0);
    ...
    betas.forEach((bk, idx) => { betaValues[`beta_${idx}`] = bk; });
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '共振幅频', formula: 'A(f) = (F0/m) / sqrt((omega_0^2 - omega^2)^2 + (2*beta*omega)^2)', variables: {} },
          { name: '峰值频率', formula: 'omega_peak = sqrt(omega_0^2 - 2*beta^2)', variables: { peakF: { value: peakF, unit: 'Hz' } } },
