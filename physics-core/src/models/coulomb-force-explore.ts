...
export class CoulombForceExploreModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    let F_inv_r2_points: Array<{ x: number; y: number }> = [];
    ...
    if (mode === 'varyQ') {
      ...
      const qMin = qRange[0]!;
      const qMax = qRange[1]!;
      const slope = K_COULOMB * q2C / (rM * rM);     // N/C
      ...
      for (let i = 0; i <= sampleCount; i++) {
        ...
        F_inv_r2_points.push({ x: parseFloat((1 / (r_i_M * r_i_M)).toFixed(2)), y: parseFloat(F_i.toFixed(6)) });
      }
      ...
      keyframes.push(
        ...
        {
          label: '1/r² 斜率', t: duration / 2,
          position: { x: 0, y: slope },
          velocity: { x: invR2Max - invR2Min, y: F_inv_r2_points[F_inv_r2_points.length - 1]!.y - F_inv_r2_points[0]!.y },
          description: `F-1/r² 斜率 = k·q₁·q₂ = ${slope.toExponential(3)} N·m²`,
        },
        {
          label: 'r 最大', t: duration,
    ...
    const F_q: ChartSeries = {
      xLabel: '电荷 q (μC)', yLabel: '静电力 F (N)', xUnit: 'μC', yUnit: 'N',
      points: F_q_points,
    };
    const F_inv_r2: ChartSeries = {
      xLabel: '1/r² (m⁻²)', yLabel: '静电力 F (N)', xUnit: 'm⁻²', yUnit: 'N',
    ...
    const slope_Fq = mode === 'varyQ' ? ((F_q_points[F_q_points.length - 1]!.y - F_q_points[0]!.y) / (1e-6 * (F_q_points[F_q_points.length - 1]!.x - F_q_points[0]!.x))) : K_COULOMB * q2C / (rM * rM);
    const slope_FinvR2 = mode === 'varyR' ? ((F_inv_r2_points[F_inv_r2_points.length - 1]!.y - F_inv_r2_points[0]!.y) / (F_inv_r2_points[F_inv_r2_points.length - 1]!.x - F_inv_r2_points[0]!.x)) : K_COULOMB * q1C * q2C;
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '库仑定律', formula: 'F = k·|q₁q₂|/r²', variables: { k: { value: K_COULOMB, unit: 'N·m²/C²' }, 'q₁': { value: q1C, unit: 'C' }, 'q₂': { value: q2C, unit: 'C' }, r: { value: rM, unit: 'm' } } },
          { name: 'F-q 斜率', formula: 'slope = k·q₂/r²', variables: { slope: { value: slope_Fq, unit: 'N/C' } } },
          { name: 'F-1/r² 斜率', formula: 'slope = k·q₁q₂', variables: { slope: { value: slope_FinvR2, unit: 'N·m²' } } },
        ],
      },
      renderHints: [
