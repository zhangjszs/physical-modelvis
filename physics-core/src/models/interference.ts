...
export class InterferenceModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const intensityCurve: ChartSeries = {
      xLabel: '屏上位置 x (mm)', yLabel: '相对光强 I/I₀', xUnit: 'mm', yUnit: '',
      points: [],
    };
    for (let i = 0; i <= N; i++) {
      const x = -w / 2 + (w * i) / N;     // m
    ...
    const fringeOrderCurve: ChartSeries = {
      xLabel: '屏上位置 x (mm)', yLabel: '光程差 Δ/λ', xUnit: 'mm', yUnit: '',
      points: [],
    };
    for (let i = 0; i <= 200; i++) {
      const x = -w / 2 + (w * i) / 200;
    ...
    while (true) {
      ...
      if (k === 0) {
        fringes.push({ t: 0, position: { x: xK, y: 0 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 });
      } else {
        fringes.push({ t: 0, position: { x: xK, y: 0 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 });
        fringes.push({ t: 0, position: { x: -xK, y: 0 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 });
      }
    ...
    const steps: ExplanationStep[] = [
      ...
      },
      {
        ...
        calculation: k > 1 ? `k=±1: x=±${(deltaY * 1e3).toFixed(3)}mm; k=±2: x=±${(2 * deltaY * 1e3).toFixed(3)}mm` : '仅中央明纹可见',
      },
      {
        order: 3,
        description: '光强分布',
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '条纹间距', formula: 'Δy = λ·L/d', variables: { λ: { value: lambdaNm, unit: 'nm' }, L: { value: L, unit: 'm' }, d: { value: dMm, unit: 'mm' }, 'Δy': { value: deltaYmm, unit: 'mm' } } },
          { name: '明纹位置', formula: 'x_k = k·λ·L/d', variables: { k: { value: 1, unit: '' }, 'x₁': { value: deltaYmm, unit: 'mm' } } },
        ],
      },
      errors: [],
      warnings,
