...
export class ReactionTimeModel extends PhysicsModelBase {
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        const h_t_series: ChartSeries = {
            ...
            xUnit: 's',
            yUnit: 'm',
            points: trajectory.filter(p => p.t <= reactionTime * 1.01).map(p => ({ x: p.t, y: 0.5 * g * p.t * p.t }))
        };
        ...
        return {
            ...
            diagnostics: {
                ...
                rangeCheck: {
                    ...
                    warnings: h < 0.05 || h > 0.5 ? [`下落距离 h=${h}m 超出推荐范围 [0.05, 0.5] m，结果仅供参考`] : []
                }
            },
            explanation: {
                ...
                formulas: [
                    {
                        ...
                        variables: {
                            h: { value: h, unit: 'm' },
                            g: { value: g, unit: 'm/s²' },
                            t: { value: reactionTime, unit: 's' }
                        }
                    },
                    {
                        name: '反应时间',
                        formula: ReactionTimeModel.REACTION_TIME_FORMULA,
                        variables: {
                            h: { value: h, unit: 'm' },
                            g: { value: g, unit: 'm/s²' },
                            t: { value: reactionTime, unit: 's' }
                    ...
                    },
                    {
                        ...
                        formula: 'σ_t = σ_h / √(2hg)',
                        variables: {
                            sigma_h: { value: sigmaH, unit: 'm' },
                            h: { value: h, unit: 'm' },
                            g: { value: g, unit: 'm/s²' },
                            sigma_t: { value: sigmaT, unit: 's' }
                        }
                    }
                ]
            },
