import type { PhysicsProblem, CenterOfGravityConstraint } from '../types/problem.js';
...
export class CenterOfGravityModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'suspensionIndex1', description: '第一次悬挂点索引', unit: '', required: false, defaultValue: 0, min: 0 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const staticDiagram: ChartSeries = {
      xLabel: 'x', yLabel: 'y', xUnit: 'm', yUnit: 'm',
      points,
    };

    // 公式与说明
    const formulaCx = '(1/6A)·Σ(xᵢ+xᵢ₊₁)(xᵢyᵢ₊₁ - xᵢ₊₁yᵢ)';
    const formulaCy = '(1/6A)·Σ(yᵢ+yᵢ₊₁)(xᵢyᵢ₊₁ - xᵢ₊₁yᵢ)';
