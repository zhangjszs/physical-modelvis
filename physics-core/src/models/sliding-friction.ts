...
export class SlidingFrictionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const f_N: ChartSeries = {
      xLabel: '正压力 N', yLabel: '滑动摩擦力 f', xUnit: 'N', yUnit: 'N',
    ...
    return {
      ...
      explanation: {
        ...
        steps: [
          { order: 1, description: '正压力 (水平面)', formula: 'N = mg', calculation: `N = ${m}×${g} = ${N.toFixed(3)} N` },
          { order: 2, description: '滑动摩擦力', formula: 'f = μN = μmg', calculation: `f = ${mu}×${N.toFixed(3)} = ${f.toFixed(3)} N` },
          { order: 3, description: uniformMotion ? '匀速条件' : '加速条件', formula: uniformMotion ? 'F_pull = f' : 'F_pull > f', calculation: uniformMotion ? `F_pull = ${f.toFixed(3)} N` : `F_pull = ${F_pull.toFixed(3)} N > f = ${f.toFixed(3)} N` },
          { order: 4, description: '动摩擦因数', formula: 'μ = f/N', calculation: `μ = ${f.toFixed(3)}/${N.toFixed(3)} = ${mu}`, result: '由接触面材料和粗糙程度决定' },
        ],
        formulas: [
          { name: '滑动摩擦力', formula: 'f = μN', variables: { μ: { value: mu, unit: '' }, N: { value: N, unit: 'N' } } },
          { name: '正压力 (水平面)', formula: 'N = mg', variables: { m: { value: m, unit: 'kg' }, g: { value: g, unit: 'm/s²' } } },
          { name: '动摩擦因数', formula: 'μ = f/N', variables: { f: { value: f, unit: 'N' }, N: { value: N, unit: 'N' } } },
          { name: '匀速条件', formula: 'F_pull = f', variables: { F_pull: { value: F_pull, unit: 'N' }, f: { value: f, unit: 'N' } } },
        ],
      },
      errors: [],
      warnings: mu > 1 ? ['动摩擦因数 μ>1 不常见，请确认接触面材料'] : [],
