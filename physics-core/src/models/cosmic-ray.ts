...
export class CosmicRayModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const h = c.altitude, mode = c.shieldingMode, thick = c.shieldThickness ?? 50;
    ...
    for (let hi = 0; hi <= 30000; hi += 1000) {
      const rateHi = rateSeaLevel * (hi < hScale ? 1 + hi / 1000 * 0.5 : Math.exp((hi - hScale) / hScale) * 1.5);
    ...
    return {
      meta: { model: 'cosmic-ray', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '当前', t: 0, position: { x: h, y: rateAtAlt }, velocity: { x: 0, y: 0 }, description: `h=${h}m, rate=${rateAtAlt.toFixed(2)}` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { h, rateAtAlt, rateShielded, mu }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `宇宙射线: h=${h}m, rate=${rateAtAlt.toFixed(2)}, shield=${mode}(${thick}cm), trans=${(rateShielded / rateAtAlt * 100).toFixed(1)}%`, steps: [
        { order: 1, description: '高度关系', formula: 'I(h) ∝ exp(h/h_scale)', result: `h_scale=${hScale}m` },
        { order: 2, description: '屏蔽衰减', formula: 'I = I0·exp(-μx)', result: `μ=${mu} cm⁻¹` },
      ], formulas: [{ name: '指数衰减', formula: 'I = I0·exp(-μx)', variables: { mu: { value: mu, unit: 'cm⁻¹' } } }] },
      errors: [], warnings: [],
    };
  }
}
