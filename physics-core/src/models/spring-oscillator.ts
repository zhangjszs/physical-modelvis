  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const v_t: ChartSeries = {
      ...
      points: trajectory.map(p => {
        const disp = Vec2.magnitude(Vec2.sub(p.position, anchor)) - L0;
        const dir = Vec2.magnitude(Vec2.sub(p.position, anchor)) > 0
    ...
    const dampingLabel = dampingType === 'none' ? '无阻尼'
      : dampingType === 'underdamped' ? '欠阻尼'
      : dampingType === 'critical' ? '临界阻尼'
      : '过阻尼';
        ...
        steps: beta === 0
          ...
          : dampingType === 'underdamped'
            ...
            : dampingType === 'critical'
              ? [
                  { order: 1, description: '临界阻尼条件', formula: 'β = ω₀', calculation: `β = ${beta.toFixed(4)}, ω₀ = ${omega0.toFixed(4)}` },
                  { order: 2, description: '运动方程', formula: 'x(t) = (C₁ + C₂t)·e^(-βt)' },
                ]
              : [
                  { order: 1, description: '过阻尼条件', formula: 'β > ω₀', calculation: `β = ${beta.toFixed(4)} > ω₀ = ${omega0.toFixed(4)}` },
