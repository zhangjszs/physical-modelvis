...
export class DoublePendulumSyncModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'phaseDiff', description: '相位差 phi2-phi1 (度) — 0=同相, 180=反相', unit: 'deg', required: true, min: 0, max: 360 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (let i = 0; i <= sampleCount; i++) {
      ...
      const dth2 = -th2Amp * omega2 * Math.sin(omega2 * t + phaseDiffRad);
    ...
    const x2 = pivot.x + L2 * Math.sin(th2Amp * Math.cos(phaseDiffRad));
    const y2 = pivot.y + L2 * Math.cos(th2Amp * Math.cos(phaseDiffRad));
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '单摆周期', formula: 'T = 2*pi*sqrt(L/g)', variables: { L1: { value: L1, unit: 'm' }, T1: { value: T1, unit: 's' }, L2: { value: L2, unit: 'm' }, T2: { value: T2, unit: 's' } } },
