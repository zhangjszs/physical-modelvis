...
export class ACCurrentModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const turnsRatioInv = turnsRatio > 0 ? 1 / turnsRatio : 0;
    const U2 = turnsRatio > 0 ? Em * turnsRatio : 0; // 次级峰值
    const U2eff = U2 / Math.sqrt(2);

    // e-t 曲线
    const e_t: ChartSeries = {
      xLabel: '时间 t (ms)', yLabel: '电动势 e (V)', xUnit: 'ms', yUnit: 'V',
      points: [],
    };
    const steps = 200;
    ...
    const u2_t: ChartSeries = {
      xLabel: '时间 t (ms)', yLabel: '次级电压 u₂ (V)', xUnit: 'ms', yUnit: 'V',
      points: [],
    };
    if (turnsRatio > 0) {
      for (let i = 0; i <= steps; i++) {
    ...
    const stepsExpl: ExplanationStep[] = [
      { order: 1, description: '交变电动势', formula: 'e = Eₘ·sin(ωt)', calculation: `Eₘ=${Em} V, ω=${omega} rad/s, f=${f.toFixed(1)} Hz` },
      { order: 2, description: '有效值', formula: 'E_eff = Eₘ/√2', calculation: `E_eff = ${Em}/1.414 = ${Eeff.toFixed(2)} V` },
    ];
    if (turnsRatio > 0) {
      stepsExpl.push({ order: 3, description: '理想变压器', formula: 'U₁/U₂ = n₁/n₂', calculation: `U₂ = ${Em}×${turnsRatio.toFixed(2)} = ${U2.toFixed(2)} V` });
    }

    return {
      meta: { model: 'ac-current', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps: stepsExpl,
        formulas: [
          { name: '交变电流', formula: 'e=Eₘ·sinωt', variables: { Em: { value: Em, unit: 'V' }, ω: { value: omega, unit: 'rad/s' }, f: { value: f, unit: 'Hz' } } },
