...
export class MicrometerModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const fixedScale: ChartSeries = {
      xLabel: '固定套筒位置 (mm)', yLabel: '刻度线', xUnit: 'mm', yUnit: '',
    ...
    const movableScale: ChartSeries = {
      xLabel: '可动刻度格数', yLabel: '对齐基准线', xUnit: '格', yUnit: '',
    ...
    return {
      meta: { model: 'micrometer', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '螺旋测微器读数', formula: 'L = a + b + n×0.01mm', variables: { L: { value: reading, unit: 'mm' }, a: { value: a, unit: 'mm' }, b: { value: b, unit: 'mm' }, n: { value: n, unit: '格' } } },
