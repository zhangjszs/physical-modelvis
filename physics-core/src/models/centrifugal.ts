...
export class CentrifugalModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    for (let i = 0; i <= sampleCount; i++) {
      ...
      if (!slipping) {
        ...
        trajectory.push({
          t,
          position: { x, y },
          velocity: { x: -omega * newR * Math.sin(newPhi) + vExcess * Math.cos(newPhi), y: omega * newR * Math.cos(newPhi) + vExcess * Math.sin(newPhi) },
    ...
    keyframes.push({
      ...
      description: willSlip
        ? `t=${duration}s 物块已脱离转盘, 沿切向-径向外滑`
        : `t=${duration}s 物块做完整圆周`,
    });
    ...
    const omega_critical_curve: ChartSeries = {
      xLabel: '半径 r (m)', xUnit: 'm', yLabel: '临界角速度 ω_crit (rad/s)', yUnit: 'rad/s',
    ...
    const required_vs_provided: ChartSeries = {
      xLabel: '半径 r (m)', xUnit: 'm', yLabel: '向心力 F_需 (N)', yUnit: 'N',
      points: Array.from({ length: 80 }, (_, i) => {
        const rr = 0.1 + (5 - 0.1) * i / 79;
    ...
    const slip_diagnostics: ChartSeries = {
      xLabel: '半径 r (m)', xUnit: 'm', yLabel: '安全系数 = F_实,max / F_需', yUnit: '',
      points: Array.from({ length: 80 }, (_, i) => {
        const rr = 0.1 + (5 - 0.1) * i / 79;
    ...
    const forceDiagram: ForceDiagram = {
      ...
      forces: [
        { name: '静摩擦提供的向心力 F_f', vector: { x: -diagramForce, y: 0 }, magnitude: diagramForce, unit: 'N' },
    ...
    return {
      ...
      explanation: {
        ...
        formulas: [
          { name: '所需向心力', formula: 'F_需 = m·ω²·r', variables: { m: { value: m, unit: 'kg' }, ω: { value: omega, unit: 'rad/s' }, r: { value: r0, unit: 'm' }, 'F_需': { value: F_required, unit: 'N' } } },
          { name: '最大静摩擦', formula: 'F_实,max = μ·m·g', variables: { μ: { value: mu, unit: '' }, m: { value: m, unit: 'kg' }, g: { value: g, unit: 'm/s²' }, 'F_实,max': { value: F_fricMax, unit: 'N' } } },
          { name: '临界角速度', formula: 'ω_crit = √(μ·g/r)', variables: { ω_crit: { value: omegaCrit, unit: 'rad/s' }, μ: { value: mu, unit: '' }, g: { value: g, unit: 'm/s²' }, r: { value: r0, unit: 'm' } } },
        ],
      },
      errors: [],
      warnings: willSlip
        ? [`物块做离心运动: t=${ slipStartT > 0 ? slipStartT.toFixed(3) : '0' }s 起滑动`]
        : [],
    };
  }
}
