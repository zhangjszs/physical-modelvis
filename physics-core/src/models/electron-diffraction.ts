...
export class ElectronDiffractionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const h = 6.626e-34, m = 9.109e-31, e = 1.602e-19;
    const lam = h / Math.sqrt(2 * m * e * U);
    const N = c.sampleCount ?? 50;

    const x_t: ChartSeries = { xLabel: '加速电压 U (V)', yLabel: '波长 λ (nm)', xUnit: 'V', yUnit: 'nm', points: [] };
    ...
    return {
      meta: { model: 'electron-diffraction', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '第一衍射环', t: 0, position: { x: 1, y: y_t.points[0]?.y ?? 0.1 }, velocity: { x: 0, y: 0 }, description: `U=${U}V, λ=${(lam * 1e9).toFixed(3)}nm` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { U, lambdaNm: lam * 1e9, d_nm: d * 1e9 }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `电子衍射: U=${U}V, λ=${(lam * 1e9).toFixed(3)}nm, d=${(d * 1e9).toFixed(3)}nm`, steps: [
        { order: 1, description: '德布罗意波长', formula: 'λ = h/√(2meU)', result: `λ = ${(lam * 1e9).toFixed(3)} nm` },
        { order: 2, description: '布拉格条件', formula: '2d·sinθ = nλ', result: '衍射环位置由布拉格条件决定' },
      ], formulas: [{ name: '德布罗意', formula: 'λ = h/√(2meU)', variables: { U: { value: U, unit: 'V' }, lambda: { value: lam * 1e9, unit: 'nm' } } }] },
      errors: [], warnings: [],
    };
  }
}
