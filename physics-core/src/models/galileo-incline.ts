...
export class GalileoInclineModel extends PhysicsModelBase {
  ...
  readonly description = '伽利略"冲淡重力"理想实验 (必修一第四章) — 斜面运动 + 对接斜面能量守恒 + 水平外推牛顿第一定律';
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'inclineLength', description: '斜面长度', unit: 'm', required: false, defaultValue: 2, min: 0.1, max: 100 },
  ];
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const theta_a: ChartSeries = {
      xLabel: 'θ (度)', yLabel: '加速度 a', xUnit: '°', yUnit: 'm/s²',
    ...
    const sin_theta_t_end: ChartSeries = {
      xLabel: 'sinθ', yLabel: '下滑时间 t_end', xUnit: '', yUnit: 's',
    ...
    return {
      ...
      explanation: {
        summary: `伽利略"冲淡重力": θ = ${angleDeg}°, a = g·sin${angleDeg}° = ${a.toFixed(3)} m/s², L = ${L} m, `
        ...
        formulas: [
          { name: '沿斜面加速度', formula: 'a = g·sinθ', variables: { g: { value: g, unit: 'm/s²' }, 'θ': { value: angleDeg, unit: '°' }, 'sinθ': { value: sinTheta, unit: '' } } },
          { name: '斜面位移', formula: 'x(t) = ½·g·sinθ·t²', variables: { g: { value: g, unit: 'm/s²' }, 'sinθ': { value: sinTheta, unit: '' } } },
          { name: '底部速度 (能量守恒)', formula: 'v = √(2·g·L·sinθ) = √(2·g·h)', variables: { '2·g·h': { value: 2 * g * h, unit: 'm²/s²' }, v: { value: vEnd, unit: 'm/s' } } },
          { name: '下滑时间', formula: 't_end = √(2·L / (g·sinθ))', variables: { L: { value: L, unit: 'm' }, 'g·sinθ': { value: a, unit: 'm/s²' }, t_end: { value: tEnd, unit: 's' } } },
  ...
  private buildDockedTrajectory(opts: {
    ...
    const aMag = Math.sqrt(dockedAccelerationVec.x * dockedAccelerationVec.x + dockedAccelerationVec.y * dockedAccelerationVec.y);
