import type { PhysicsProblem, InertiaConstraint, InertiaMode } from '../types/problem.js';
...
export class InertiaModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'massRatio', description: '质量比 m_top/m_bottom', unit: '', required: false, defaultValue: 0.1, min: 0, max: 10 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    if (mode === 'stroke') {
      ...
      steps = [
        { order: 1, description: '惯性定义', formula: '物体保持原运动状态的性质', calculation: '质量越大, 惯性越大' },
        { order: 2, description: '打击分析', formula: '打击时间极短, 力仅作用于下方棋子', calculation: '上方棋子水平不受力' },
        { order: 3, description: '上方棋子水平运动', formula: 'x_top(t) = x₀ (水平惯性)', calculation: `x_top(t) = ${startTop.x} (恒定)` },
        { order: 4, description: '上方棋子竖直运动', formula: 'y_top(t) = y₀ - ½gt² (自由落体)', calculation: `从 y₀=${startTop.y}m 落到 y=0 用时 t=√(2y₀/g)=${tFall.toFixed(3)}s` },
        { order: 5, description: '下方棋子减速', formula: 'a = -μmg/m = -μg', calculation: `a = -${mu}×${g} = ${a_friction.toFixed(2)} m/s², 经 t=${tStop.toFixed(3)}s 停止` },
      ];
      formulas = [
        { name: '惯性定义', formula: '质量是惯性大小的唯一量度', variables: { m_top: { value: mTop, unit: 'kg' }, m_bottom: { value: mBottom, unit: 'kg' } } },
        { name: '自由落体', formula: 'y(t) = y₀ - ½gt²', variables: { y0: { value: startTop.y, unit: 'm' }, g: { value: g, unit: 'm/s²' } } },
        { name: '滑动摩擦', formula: 'f = μN = μmg, a = -μg', variables: { μ: { value: mu, unit: '' }, m: { value: mBottom, unit: 'kg' }, g: { value: g, unit: 'm/s²' } } },
        { name: '水平惯性', formula: '水平方向无力 → vx 保持不变', variables: { vx: { value: 0, unit: 'm/s' } } },
      ];
    } else if (mode === 'stop') {
      // === 小车急停木块倒伏 ===
      ...
      steps = [
        { order: 1, description: '惯性定义', formula: '物体保持原运动状态的性质', calculation: '木块原以 v₀ 向右运动' },
        { order: 2, description: '小车急停', formula: 'C → v_C = 0, a_stop = -v₀/t_stop', calculation: `a_stop = ${aStop.toFixed(2)} m/s²` },
        { order: 3, description: '木块底部', formula: '底部与小车接触 → 随小车停止', calculation: `底部与小车一同经 ${tStop}s 停下` },
        { order: 4, description: '木块顶部惯性', formula: '顶部保持原速度 v₀ (水平惯性)', calculation: `v_top = ${initialSpeed} m/s 不变` },
        { order: 5, description: '倒伏原因', formula: '底部停 + 顶部继续 → 倾倒', calculation: '倒伏方向: 向前 (运动方向)' },
      ];
      formulas = [
        { name: '惯性', formula: 'F_net = 0 → v 保持不变', variables: { v0: { value: initialSpeed, unit: 'm/s' } } },
        { name: '急停减速度', formula: 'a = (v - v₀) / t', variables: { v: { value: 0, unit: 'm/s' }, v0: { value: initialSpeed, unit: 'm/s' }, t: { value: tStop, unit: 's' } } },
      ];
    } else {
      // === smoothPull (鸡蛋落入水杯) ===
      // 初始: 硬纸板上放鸡蛋, 水杯在纸板正下方
      ...
      steps = [
        ...
        { order: 2, description: '纸板抽出', formula: '纸板受快速水平力 → 飞出', calculation: `纸板速度 v=${vCard}m/s` },
        { order: 3, description: '鸡蛋水平惯性', formula: '鸡蛋水平不受力 → x 恒定', calculation: `x_egg(t) = ${x0}` },
        { order: 4, description: '鸡蛋竖直落体', formula: 'y(t) = y₀ - ½gt²', calculation: `从 y₀=${y0}m 落到 y=0 用时 t=${tFall.toFixed(3)}s` },
        { order: 5, description: '结论', formula: '鸡蛋落入水中正下方', calculation: '水平惯性体现: 抽出纸板不影响鸡蛋水平位置' },
      ];
      formulas = [
        { name: '惯性 (静止物体)', formula: '静止 → a = 0, x 恒定', variables: { x: { value: x0, unit: 'm' } } },
        { name: '自由落体', formula: 'y(t) = y₀ - ½gt²', variables: { y0: { value: y0, unit: 'm' }, g: { value: g, unit: 'm/s²' } } },
        { name: '下落时间', formula: 't = √(2y₀/g)', variables: { y0: { value: y0, unit: 'm' }, g: { value: g, unit: 'm/s²' }, t: { value: tFall, unit: 's' } } },
      ];
    }

    // 图表: v-t (顶部物体速度-时间)
    const v_t: ChartSeries = {
      xLabel: '时间 t', yLabel: '|v| (惯性现象速度)', xUnit: 's', yUnit: 'm/s',
      points: v_t_points,
    };

    // 图表: x-t (位置-时间)
    const x_t: ChartSeries = {
      xLabel: '时间 t', yLabel: '位置', xUnit: 's', yUnit: 'm',
      points: x_t_points,
    };
    ...
    return {
      ...
      diagnostics: {
        ...
        rangeCheck: {
          withinRange: true,
          warnings: massRatio <= 0 ? ['质量比应大于 0'] : massRatio > 5 ? ['质量比过大, 不符合常见实验设置'] : [],
        },
      },
      explanation: {
        summary,
      ...
      renderHints: [
        { bodyId: body0?.id ?? 'top', renderLabel: mode === 'stroke' ? '上方棋子' : mode === 'stop' ? '木块顶部' : '鸡蛋', renderColor: '#e74c3c' },
        { bodyId: problem.bodies[1]?.id ?? 'bottom', renderLabel: mode === 'stroke' ? '下方棋子' : mode === 'stop' ? '小车' : '纸板', renderColor: '#3498db' },
      ],
      errors: [],
      warnings: [],
    };
