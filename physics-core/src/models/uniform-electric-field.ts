    ...
    return {
      ...
      diagnostics: {
        conservedQuantities: [{
          ...
          finalValue: (trajectory[trajectory.length - 1]?.kineticEnergy ?? 0) + (trajectory[trajectory.length - 1]?.potentialEnergy ?? 0),
          maxDeviation: 0,
          tolerance: 1e-6,
          conserved: true,
        }],
      ...
      explanation: {
        ...
        steps: [
          { order: 1, description: '电场力', formula: 'F = qE', calculation: `F = ${q} × (${E.x}, ${E.y}) = (${(q * E.x).toExponential(2)}, ${(q * E.y).toExponential(2)}) N` },
          { order: 2, description: '加速度', formula: 'a = qE/m', calculation: `a = (${ax.toExponential(2)}, ${ay.toExponential(2)}) m/s²` },
          { order: 3, description: '水平位移', formula: 'x = x₀ + v₀x·t' },
          { order: 4, description: '竖直位移', formula: 'y = y₀ + v₀y·t + ½at²' },
        ],
        formulas: [
          { name: '电场力', formula: 'F = qE', variables: { q: { value: q, unit: 'C' }, E: { value: Vec2.magnitude(E), unit: 'N/C' } } },
          { name: '加速度', formula: 'a = qE/m', variables: { a: { value: Vec2.magnitude(a), unit: 'm/s²' } } },
          { name: '电势能', formula: 'Ep = -qEy', variables: { q: { value: q, unit: 'C' }, E: { value: E.y, unit: 'N/C' } } },
        ],
      },
      errors: [],
      warnings: [],
