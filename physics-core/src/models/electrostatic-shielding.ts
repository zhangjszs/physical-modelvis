...
export class ElectrostaticShieldingModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const grounding_effect: ChartSeries = {
      xLabel: '外部电场 E (V/m)', yLabel: '箔片张角 θ (°)', xUnit: 'V/m', yUnit: '°',
    ...
    const field_section: ChartSeries = {
      xLabel: '位置 x (cm)', yLabel: '电场强度 E (V/m)', xUnit: 'cm', yUnit: 'V/m',
      points: Array.from({ length: sampleCount + 1 }, (_, i) => {
    ...
    const steps: ExplanationStep[] = [
      ...
      { order: 2, description: '接地屏蔽', formula: '接地 → 外表面电荷入地 → 内部无感应', result: isGrounded ? '已接地: 完全屏蔽' : '未接地: 部分屏蔽' },
      { order: 3, description: '箔片张角', formula: 'θ ∝ q_净', calculation: `θ_未接地=${thetaUngrounded.toFixed(1)}°, θ_接地=${thetaGrounded}°` },
      { order: 4, description: '空腔内部电荷影响', formula: '空腔内电荷会在外表面感应等量电荷', result: `cavityCharge=${cavityChargeMuC} μC` },
    ];

    return {
      meta: { model: 'electrostatic-shielding', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      diagnostics: {
        conservedQuantities: [],
        maxValues: {
          thetaUngrounded, thetaGrounded, E, isGrounded: isGrounded ? 1 : 0,
          cavityChargeMuC,
        },
