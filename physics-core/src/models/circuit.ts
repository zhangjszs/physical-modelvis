...
export class CircuitModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const U_R_curve: ChartSeries = {
      xLabel: '外电阻 R (Ω)', yLabel: '路端电压 U (V)', xUnit: 'Ω', yUnit: 'V',
      points: [],
    };
    ...
    const P_R_curve: ChartSeries = {
      xLabel: '外电阻 R (Ω)', yLabel: '输出功率 P (W)', xUnit: 'Ω', yUnit: 'W',
      points: [],
    };
    let pMax = 0;
    let rOpt = 0;
    ...
    const VI_curve: ChartSeries = {
      xLabel: '电压 U (V)', yLabel: '电流 I (A)', xUnit: 'V', yUnit: 'A',
      points: [],
    };
    ...
    const steps: ExplanationStep[] = [
      {
        order: 1,
        description: '等效电阻',
        formula: resistors.length === 1
          ? 'R_eq = R₁'
          : (isPureSeries ? 'R_eq = ΣR_i (串联)' : (isPureParallel ? '1/R_eq = Σ(1/R_i) (并联)' : '串并联混合计算')),
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '全电路欧姆定律', formula: 'I = E/(R+r)', variables: { E: { value: E, unit: 'V' }, R: { value: req, unit: 'Ω' }, r: { value: r, unit: 'Ω' }, I: { value: I, unit: 'A' } } },
