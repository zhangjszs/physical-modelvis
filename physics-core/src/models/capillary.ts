...
export class CapillaryModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const hCurve: ChartSeries = {
      xLabel: '1/r (1/mm)', yLabel: '毛细高度 h (mm)', xUnit: '1/mm', yUnit: 'mm',
      points: [],
    };
    const nScan = 50;
    ...
    const meniscusCurve: ChartSeries = {
      xLabel: '水平位置 x (mm)', yLabel: '液面高度 y (mm)', xUnit: 'mm', yUnit: 'mm',
      points: [],
    };
    const nMeniscus = 50;
    ...
    for (let i = 0; i <= nMeniscus; i++) {
      ...
      const y = insideRoot > 0
        ? (cosTheta > 0 ? h - (R - Math.sqrt(insideRoot)) : -(R - Math.sqrt(insideRoot)))
        : (cosTheta > 0 ? h : 0);
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '毛细高度', formula: 'h=2σcosθ/(ρgr)', variables: { sigma: { value: sigma, unit: 'N/m' }, cosTheta: { value: cosTheta, unit: '' }, rho: { value: rho, unit: 'kg/m³' }, g: { value: g, unit: 'm/s²' }, r: { value: r, unit: 'm' } } },
          { name: '接触角', formula: 'theta (材料决定)', variables: { theta: { value: thetaDeg, unit: '°' } } },
        ],
      },
      errors: [],
      warnings,
