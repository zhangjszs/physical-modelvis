...
export class SecurityAlarmModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    // — 判断干簧管状态 —
    const reedState: ReedState = d <= operateDistance ? 'closed' : (d >= releaseDistance ? 'open' : (door === 'closed' ? 'closed' : 'open'));
    ...
    const stateVsDist: ChartSeries = {
      xLabel: '磁体距离 d (mm)', yLabel: '干簧管状态 (1=闭合, 0=断开)', xUnit: 'mm', yUnit: '',
      points: [],
    };
    const distMax = 60;
    ...
    // — 真值表散点图 (6 种状态) —
    const truthTable: ChartSeries = {
      xLabel: '门状态 (0=关闭, 1=打开)', yLabel: '磁体距离 (mm)', xUnit: '门状态', yUnit: 'mm',
      points: [
        { x: 0, y: 5 },
        { x: 0, y: 10 },
        { x: 1, y: 80 },
    ...
    const circuitTable: ChartSeries = {
      xLabel: '节点序', yLabel: '电平 (V)', xUnit: '', yUnit: 'V',
      points: [
        // Row 1-2: 电源输入
        { x: 1, y: 12 },   // 电源 Vcc
        { x: 2, y: 12 },   // 电源输出
