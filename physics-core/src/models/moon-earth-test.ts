...
export class MoonEarthTestModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'moonPeriod',  description: '月球周期 T (s)',    unit: 's', required: true, min: 10 * 86400, max: 35 * 86400 },
  ];
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const c = problem.constraints?.moonEarthTest
      ?? { earthRadius: R_EARTH_STD, moonDistance: R_MOON_STD, moonPeriod: T_MOON_STD };
    ...
    const steps = this.buildSteps(R, r, T, aMoon, omega, ratioRr, gOver3600, expectedRatio, squareInv, aFromSquareInv, relDiff, pass);
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '月球向心加速度', formula: 'a_月 = ω²r = 4π²r/T²', variables: { ω: { value: omega, unit: 'rad/s' }, r: { value: r, unit: 'm' }, T: { value: T, unit: 's' } } },
          { name: '平方反比律', formula: 'a_月 / g = (R/r)²',            variables: { R: { value: R, unit: 'm' }, r: { value: r, unit: 'm' } } },
          { name: '理论值', formula: 'a_月(理论) = g·(R/r)²',           variables: { g: { value: G_STD, unit: 'm/s²' } } },
        ],
      },
      renderHints: [
      ...
      warnings: pass
        ? []
        : [`最大偏差 ${relDiff.toFixed(2)}%, 建议核对 R, r, T 输入值`],
    };
  }

  private buildSteps(
    R: number, r: number, T: number,
    aMoon: number, omega: number, ratioRr: number,
    gOver3600: number, expectedRatio: number,
    squareInv: number, aFromSquareInv: number,
    relDiff: number, pass: boolean,
  ) {
    const omega_calc = `ω = 2π / T = 2π / ${T.toExponential(3)}`;
