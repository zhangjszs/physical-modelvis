...
export class ForceCompositionModel extends PhysicsModelBase {
    ...
    readonly assumptions = ['两个分力为共点力', '分力大小不变，仅夹角变化', '力的合成遵循平行四边形定则'];
    readonly applicableRange = '适用于任意两个共点力的合成';
    readonly errorSources = ['实验中分力方向记录不准带来的误差', '弹簧测力计本身的示值误差'];
    ...
    solve(problem: PhysicsProblem): SimulationResult {
        ...
        const keyframes: Keyframe[] = [
            ...
            this.makeKeyframe(
                `θ = ${angleDeg.toFixed(1)}° (当前)`,
                (angleDeg / 180) * duration,
                Fmag,
                FangleDeg,
                F1,
                F2,
            ...
            this.makeKeyframe(
                'θ = 90° (垂直)',
                (90 / 180) * duration,
                Math.sqrt(F1 * F1 + F2 * F2),
                (Math.atan2(F2, F1) * 180) / Math.PI,
                F1,
                F2,
                90
            ),
        ...
        return {
            ...
            explanation: {
                ...
                steps: [
                    {
                        order: 1,
                        description: '合力大小 (余弦定理)',
                        formula: 'F = √(F1² + F2² + 2·F1·F2·cosθ)',
                        calculation
                    },
                    {
                        order: 2,
                        description: '合力方向 (与 F1 的夹角)',
                        formula: 'tanφ = F2·sinθ / (F1 + F2·cosθ)',
                        calculation: directionCalc
                    },
                    {
                        order: 3,
                        description: '特例检验',
                        formula: 'θ=0°: F=F1+F2; θ=180°: F=|F1-F2|; θ=90°: F=√(F1²+F2²)',
                        result: '平行四边形定则成立'
                    }
                ],
                formulas: [
                    ...
                    },
                    {
                        ...
                        variables: {
                            ...
                            F1: { value: F1, unit: 'N' },
                            cosθ: { value: cosTheta, unit: '' }
                        }
                    },
                    {
                        name: '同向合成',
                        formula: 'F = F1 + F2',
                        variables: { F1: { value: F1, unit: 'N' }, F2: { value: F2, unit: 'N' } }
                    },
                    {
                        name: '反向合成',
                        formula: 'F = |F1 − F2|',
    ...
    private makeKeyframe(
        ...
        return {
            ...
            t,
            position: {
                x: fMag * Math.cos((fAngleDeg * Math.PI) / 180),
                y: fMag * Math.sin((fAngleDeg * Math.PI) / 180)
            },
            velocity: { x: 0, y: 0 },
