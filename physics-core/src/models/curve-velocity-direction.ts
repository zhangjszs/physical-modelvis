...
export class CurveVelocityDirectionModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'angularSpeed', description: "角速度 ω (rad/s) 或切向初速度大小 (m/s)", unit: 'rad/s', required: true, min: 0 },
  ];
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const releaseDuration = c.releaseDuration ?? 2;
    ...
    interface TrackSample { pos: { x: number; y: number }; tangent: { x: number; y: number }; }
    ...
    const v_tang_t: ChartSeries = {
      xLabel: '时间', yLabel: '切向速度大小', xUnit: 's', yUnit: 'm/s',
    ...
    const v_norm_t: ChartSeries = {
      xLabel: '时间', yLabel: '法向速度 (理论=0)', xUnit: 's', yUnit: 'm/s',
      points: firstTraj.map(p => ({ x: p.t, y: 0 })),
    };
    // 多条子轨迹 x 对比 (图表展示 不同脱离点的 x-t)
    const x_t_multi = (() => {
      // 取每个轨迹一系列 x 点 (时间等间距)
      const pts: Array<{ x: number; y: number }> = [];
      for (let i = 0; i <= 100; i++) {
        const t = duration * i / 100;
        // 取中间轨迹 (k=floor(sampleCount/2))
        const midTraj = allTrajectories[Math.floor(sampleCount / 2)]!;
        const idx = Math.min(midTraj.length - 1, Math.floor(i / 100 * (midTraj.length - 1)));
        pts.push({ x: t, y: midTraj[idx]!.position.x });
      }
      return { xLabel: '时间', yLabel: '中央脱离点 x 坐标', xUnit: 's', yUnit: 'm', points: pts };
    })();
...
function cnShape(s: CurveTrackShape): string {
  switch (s) {
    case 'circle': return '圆周';
    case 'parabola': return '抛物线';
    case 'spiral': return '螺线';
  }
}
