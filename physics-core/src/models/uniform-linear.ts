  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    return {
      ...
      explanation: {
        ...
        steps: [
          { order: 1, description: '匀速直线运动的速度恒定', formula: 'v = v₀', result: `v = ${speed.toFixed(2)} m/s` },
          { order: 2, description: '位移公式', formula: 'x = x₀ + vt', calculation: `x = ${x0.x} + ${speed.toFixed(2)} × ${duration}`, result: `x = ${(x0.x + speed * duration).toFixed(2)} m` },
        ],
        formulas: [{
          name: '匀速直线运动位移公式',
          formula: 'x = x₀ + vt',
          variables: { 'x₀': { value: x0.x, unit: 'm' }, v: { value: speed, unit: 'm/s' }, t: { value: duration, unit: 's' } },
        }],
      },
      errors: [],
      warnings: [],
