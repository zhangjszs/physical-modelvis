...
export class DecayStatisticsModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const pseudoRand = (seed: number) => { let s = seed; return () => { s = (s * 1103515245 + 12345) & 0x7fffffff; return s / 0x7fffffff; }; };
    ...
    counts.forEach(n => { if (n <= maxCount) histogram[n]++; });
    ...
    for (let n = 0; n <= maxCount; n++) {
      ...
      const gauss = N * (1 / (sigma * Math.sqrt(2 * Math.PI))) * Math.exp(-((n - lambda) ** 2) / (2 * sigma * sigma));
      y_t.points.push({ x: n, y: parseFloat(gauss.toFixed(1)) });
      if (n < 50) trajectory.push({ t: n * 0.01, position: { x: n, y: histogram[n] ?? 0 }, velocity: { x: 0, y: 0 } });
    }

    return {
      meta: { model: 'decay-statistics', solver: 'numerical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '均值', t: 0, position: { x: lambda, y: histogram[Math.round(lambda)] ?? 0 }, velocity: { x: 0, y: 0 }, description: `N̄=${lambda}, σ=${sigma.toFixed(2)}` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { lambda, sigma, maxBin: Math.max(...histogram) }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `衰变统计: N̄=${lambda}, σ=${sigma.toFixed(2)}, n=${nTrials}次`, steps: [
      ...
      ], formulas: [{ name: '泊松', formula: 'P(N)=λ^N·e^(-λ)/N!', variables: { lambda: { value: lambda, unit: '' }, sigma: { value: sigma, unit: '' } } }] },
      errors: [], warnings: [],
    };
  }
}
