...
export class MeltingCurveModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const tempCurve: ChartSeries = {
      xLabel: '时间 t (min)', yLabel: '温度 T (°C)', xUnit: 'min', yUnit: '°C',
      points: [],
    };
    // 加热功率 (恒定)
    const powerCurve: ChartSeries = {
      xLabel: '时间 t (min)', yLabel: '加热功率 P (W)', xUnit: 'min', yUnit: 'W',
      points: [],
    };
    // 相分数 (0=全固, 1=全液)
    const phaseCurve: ChartSeries = {
      xLabel: '时间 t (min)', yLabel: '液相分数', xUnit: 'min', yUnit: '',
      points: [],
    };
    ...
    const keyframes: Keyframe[] = [
      ...
      },
      {
        ...
        position: { x: mode === 'crystal' ? parseFloat(((Tm - T0) / rate).toFixed(2)) : parseFloat((durationMin / 2).toFixed(2)), y: mode === 'crystal' ? Tm : parseFloat((T0 + rate * durationMin / 2).toFixed(2)) },
        velocity: { x: 0, y: 0 },
        description: mode === 'crystal'
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '加热公式', formula: 'T = T₀ + r·t (T<T_m)', variables: { T0: { value: T0, unit: '°C' }, r: { value: rate, unit: '°C/min' } } },
