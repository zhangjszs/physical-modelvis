  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const keyframes: Keyframe[] = [
      ...
      },
      {
        label: '第 1 次极大',
        t: 0,
        position: { x: secondaryPeaks.length > 0 ? secondaryPeaks[0]!.theta : 0, y: secondaryPeaks.length > 0 ? secondaryPeaks[0]!.intensity : 0 },
        velocity: { x: 0, y: 0 },
    ...
    const steps: ExplanationStep[] = [
      ...
      },
      {
        ...
        result: secondaryPeaks.length >= 1 ? `1st: ${(secondaryPeaks[0]!.intensity * 100).toFixed(2)}%` : 'none visible',
      },
    ];
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '强度公式', formula: 'I = I_0*(sin(alpha)/alpha)^2, alpha=pi*a*sin(theta)/lambda', variables: { a: { value: aMm, unit: 'mm' }, lambda: { value: c.wavelength, unit: 'nm' } } },
          { name: '极小位置', formula: 'sin(theta_m) = +-m*lambda/a', variables: { theta_1: { value: theta1 * 180 / Math.PI, unit: 'deg' } } },
        ],
      },
      errors: [],
      warnings,
