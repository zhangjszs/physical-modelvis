...
export class ThermistorModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const resistanceVsTemp: ChartSeries = {
      xLabel: '温度 T (K)', yLabel: '电阻 R (Ω)', xUnit: 'K', yUnit: 'Ω',
      points: [],
    };
    ...
    const lnRvsInvT: ChartSeries = {
      xLabel: '1/T (K⁻¹)', yLabel: 'ln(R)', xUnit: '1/K', yUnit: '',
      points: [],
    };
    if (mode === 'NTC') {
      for (let i = 0; i <= 80; i++) {
    ...
    const steps: ExplanationStep[] = [
      {
        ...
        formula: mode === 'NTC' ? 'R(T) = R0 * exp(B * (1/T - 1/T0))' : 'PTC: R=R0 (T<=Tc); R=R0*exp(C*(T-Tc)/Tc) (T>Tc)',
        calculation: `模式=${mode}, R0=${R0}Ω, B=${B}K, T0=${T0}K, 目标温度 T=${Ttarget}K`,
      },
      {
        order: 2,
        description: '计算目标温度电阻',
        formula: mode === 'NTC' ? 'R(T) = R0 * exp(B * (1/T - 1/T0))' : (Ttarget <= curieTemp ? 'R = R0' : 'R = R0 * exp(C * (T - Tc) / Tc)'),
      ...
      },
      {
        ...
        calculation: mode === 'NTC' ? `α ≈ -B/T² = ${( -B / (Ttarget * Ttarget) * 100).toFixed(4)} %/K` : `PTC 在居里点附近温度系数急剧变号`,
      ...
      },
      {
        ...
        result: mode === 'NTC' ? '温度升高 → R 降低 → 分压变化 → 比较器翻转 → 继电器动作' : 'PTC 自恢复保险丝: 过流 → 发热 → 超过 Tc → 电阻激增 → 限流保护',
      },
    ];
    ...
    const formulas: FormulaUsage[] = [
      {
        ...
        formula: mode === 'NTC' ? 'R(T) = R0 * exp(B * (1/T - 1/T0))' : 'R(T) = R0 (T <= Tc) 或 R0*exp(C*(T-Tc)/Tc) (T > Tc)',
        variables: {
          R0: { value: R0, unit: 'Ω' },
          T0: { value: T0, unit: 'K' },
          B: { value: B, unit: 'K' },
