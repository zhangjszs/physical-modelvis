...
export class HeatDirectionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const T1 = c.hotTemp, T2 = c.coldTemp, k = c.thermalConductivity;
    const Teq = (T1 + T2) / 2;
    const tau = 10 / (k + 0.01);
    ...
    return {
      meta: { model: 'heat-direction', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '初始', t: 0, position: { x: T1, y: T2 }, velocity: { x: 0, y: 0 }, description: `T_hot=${T1}K, T_cold=${T2}K` },
        { label: '平衡', t: dur, position: { x: Teq, y: Teq }, velocity: { x: 0, y: 0 }, description: `T_eq=${Teq.toFixed(1)}K` },
      ],
      charts: { x_t, y_t },
      diagnostics: { conservedQuantities: [], maxValues: { T1, T2, Teq, k, tau }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `热传导: T_hot=${T1}K→${Teq.toFixed(1)}K, T_cold=${T2}K→${Teq.toFixed(1)}K`, steps: [
        { order: 1, description: '克劳修斯表述', formula: '热量自发从高温→低温', result: `T_eq = (${T1}+${T2})/2 = ${Teq.toFixed(1)}K` },
      ], formulas: [{ name: '热平衡', formula: 'T_eq = (T1+T2)/2', variables: { T1: { value: T1, unit: 'K' }, T2: { value: T2, unit: 'K' } } }] },
      errors: [], warnings: [],
    };
  }
}
