...
export class ForcedVibrationModel extends PhysicsModelBase {
  ...
  readonly requiredParameters: ParameterSpec[] = [
    ...
    { name: 'dampingBeta', description: '阻尼系数 beta = c/(2m) (1/s)', unit: '1/s', required: true, min: 0, max: 5 },
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '固有频率', formula: 'omega_0 = sqrt(k/m)', variables: { k: { value: k, unit: 'N/m' }, m: { value: m, unit: 'kg' } } },
          { name: '稳态振幅', formula: 'A = (F0/m) / sqrt((omega_0^2 - omega_d^2)^2 + (2*beta*omega_d)^2)', variables: { F0: { value: F0, unit: 'N' }, A: { value: A_theoretical, unit: 'm' } } },
        ],
      },
      errors: [],
      warnings,
