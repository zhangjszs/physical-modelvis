...
export class AlphaScatteringModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const thetaDist: ChartSeries = { xLabel: '散射角 (度)', yLabel: '粒子数', xUnit: 'deg', yUnit: '', points: Array.from({ length: 18 }, (_, i) => ({ x: (i + 1) * 10, y: 0 })) };
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (i < 5) {
        ...
        for (let s = 0; s <= 20; s++) {
          ...
          if (t < 0.5) { x = x0 + t * 2 * 100; y = y0; }
          else { x = x0 + 100 + (t - 0.5) * 2 * 100 * Math.cos(phi); y = y0 + (t - 0.5) * 2 * 100 * Math.sin(phi); }
          trajectory.push({ t: s * 0.1, position: { x: parseFloat(x.toFixed(1)), y: parseFloat(y.toFixed(1)) }, velocity: { x: 0, y: 0 } });
        }
    ...
    return {
      meta: { model: 'alpha-scattering', solver: 'numerical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      trajectories: [trajectory], keyframes: [
        { label: '入射', t: 0, position: { x: -100, y: 0 }, velocity: { x: 1, y: 0 }, description: `n=${n}, E=${E_MeV}MeV, Z=${Z}` },
      ],
      charts: { x_t: thetaDist, y_t: x_t, v_t: y_t },
      diagnostics: { conservedQuantities: [], maxValues: { E_MeV, Z, k, n }, rangeCheck: { withinRange: true, warnings: [] } },
      explanation: { summary: `α 散射: E=${E_MeV}MeV, Z=${Z}, k=${k.toFixed(2)}fm`, steps: [
        { order: 1, description: '碰撞参数', formula: 'b = (Z·e²)/(4πε₀·E) · cot(θ/2)', result: `k=${k.toFixed(2)} fm` },
        { order: 2, description: '结论', formula: '绝大多数直线穿过', result: '说明原子核极小, 核式结构' },
      ], formulas: [{ name: '碰撞参数', formula: 'b = k·cot(θ/2)', variables: { k: { value: parseFloat(k.toFixed(3)), unit: 'fm' } } }] },
      errors: [], warnings: [],
    };
  }
}
