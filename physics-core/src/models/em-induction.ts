...
export class EMInductionModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const flux_t: ChartSeries = {
      xLabel: '时间 t (ms)', yLabel: '磁通量 Φ (mWb)', xUnit: 'ms', yUnit: 'mWb',
      points: [],
    };
    const T = 1 / 50;
    const steps = 200;
    ...
    const emf_t: ChartSeries = {
      xLabel: '时间 t (ms)', yLabel: '感应电动势 ε (mV)', xUnit: 'ms', yUnit: 'mV',
      points: [],
    };
    for (let i = 0; i <= steps; i++) {
      const t = T * i / steps;
    ...
    const stepsExpl: ExplanationStep[] = [
      { order: 1, description: '磁通量', formula: 'Φ = B·A·cosθ', calculation: `Φ = ${B} × ${A} × cos${thetaDeg}° = ${(flux * 1e3).toFixed(4)} mWb` },
      { order: 2, description: '法拉第电磁感应定律', formula: 'ε = −N·dΦ/dt', result: `N=${N} 匝时 ε = ${(N * B * A * omega).toFixed(2)}·sin(ωt) V` },
    ];
    if (hasCutting) {
      stepsExpl.push({ order: 3, description: '切割情景', formula: 'ε = B·L·v', calculation: `ε = ${B}×${ec.cuttingLength}×${ec.cuttingVelocity} = ${emfCutting.toFixed(3)} V` });
    }

    return {
      meta: { model: 'em-induction', solver: 'analytical', computationTime: 0, timestamp: new Date().toISOString(), version: this.version },
      ...
      explanation: {
        ...
        steps: stepsExpl,
        formulas: [
          { name: '法拉第定律', formula: 'ε=−NdΦ/dt', variables: { N: { value: N, unit: '匝' }, B: { value: B, unit: 'T' }, A: { value: A, unit: 'm²' } } },
