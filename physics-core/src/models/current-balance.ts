...
export class CurrentBalanceModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    const B = (cb as unknown as Record<string, unknown>).magneticField ?? (cb as unknown as Record<string, unknown>).B ?? 0.5;
    const Bval = typeof B === 'number' ? B : 0.5; // T
    if (Bval <= 0) throw new Error('磁感应强度 B 必须为正');

    const armLen = ((cb as unknown as Record<string, unknown>).armLen as number) ?? 0.1; // m，天平臂长
    ...
    if (Math.abs(F_net) / F_gravity > 0.2) {
      warnings.push(`合力 ${(F_net).toFixed(3)}N 相对重力 ${(F_gravity).toFixed(3)}N 偏大，建议调整电流至 I_eq=${I_eq.toFixed(3)}A 附近`);
    }
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '安培力', formula: 'F = n·B·I·l', variables: { n: { value: n, unit: '匝' }, B: { value: Bval, unit: 'T' }, I: { value: I, unit: 'A' }, l: { value: l, unit: 'm' }, F: { value: F_ampere, unit: 'N' } } },
          { name: '平衡条件', formula: 'm·g = n·B·I·l', variables: { m: { value: m, unit: 'kg' }, g: { value: g, unit: 'm/s²' }, I_eq: { value: I_eq, unit: 'A' } } },
        ],
      },
      errors: [],
      warnings,
