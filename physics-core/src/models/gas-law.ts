...
export class GasLawModel extends PhysicsModelBase {
  ...
  solve(problem: PhysicsProblem): SimulationResult {
    ...
    if (mode === 'isothermal') {
      ...
      // p-V 曲线: p = nRT0/V
      processCurve = {
        xLabel: '体积 V (L)', yLabel: '压强 p (kPa)', xUnit: 'L', yUnit: 'kPa',
      ...
      processCurve = {
        xLabel: '温度 T (K)', yLabel: '压强 p (kPa)', xUnit: 'K', yUnit: 'kPa',
    ...
    const trajectory: TrajectoryPoint[] = [
      { t: 0, position: { x: V0 * 1e3, y: p0 / 1e3 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 },
      { t: 0, position: { x: VFinal * 1e3, y: pFinal / 1e3 }, velocity: { x: 0, y: 0 }, kineticEnergy: 0, potentialEnergy: 0 },
    ];
    ...
    const steps: ExplanationStep[] = [
      ...
      {
        order: 2,
        description: mode === 'isothermal'
          ? '玻意耳定律: pV=const (等温)'
          : mode === 'isobaric'
            ? '查理定律: V/T=const (等压)'
            : '盖-吕萨克定律: p/T=const (等容)',
        formula: mode === 'isothermal'
          ? 'p₁V₁ = p₂V₂'
          : mode === 'isobaric'
            ? 'V₁/T₁ = V₂/T₂'
            : 'p₁/T₁ = p₂/T₂',
    ...
    return {
      ...
      explanation: {
        ...
        steps,
        formulas: [
          { name: '理想气体状态方程', formula: 'pV=nRT', variables: { p: { value: p0, unit: 'Pa' }, V: { value: V0, unit: 'm³' }, n: { value: n, unit: 'mol' }, T: { value: T0, unit: 'K' }, R: { value: R, unit: 'J/(mol·K)' } } },
        ],
      },
      errors: [],
      warnings,
  ...
  /** 生成曲线采样 */
  private generateIsoCurve(xMin: number, xMax: number, N: number, fn: (x: number) => number): Array<{ x: number; y: number }> {
