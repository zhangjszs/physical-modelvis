...
function makeProblem(overrides: {
  ...
  const { mode = 'transverse', amplitude = 0.1, frequency = 2, wavelength = 0.5, duration = 3 } = overrides;
  return {
    id: 'wave-test',
    model: 'mechanical-wave',
    bodies: [{ id: 'medium', mass: { value: 0.1, unit: 'kg' }, position: { x: 0, y: 0 }, velocity: { x: 0, y: 0 } }],
    constraints: { wave: { mode, amplitude, frequency, wavelength } },
    environment: {},
...
describe('MechanicalWaveModel', () => {
  ...
  it('干涉: 存在波节 (y=0 恒定点，对向波形成的驻波)', () => {
    ...
    // 驻波波节: 在 x=0 附近质点位移始终接近 0
    const snapshot = r.trajectories[r.trajectories.length - 1]!;
    // 找 x≈0 的点
    const midPoint = snapshot.find(p => Math.abs(p.position.x) < 0.05);
    expect(midPoint).toBeDefined();
    expect(Math.abs(midPoint!.position.y)).toBeLessThan(0.02); // 波节
  });
